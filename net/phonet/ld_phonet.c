...
/* For Ramdump debugging */

static u8 ld_phonet_tx_last_buf[MAX_BUFF_SIZE]; /*Last msg recevied from modem*/
static u8 ld_phonet_rx_last_buf[MAX_BUFF_SIZE]; /*Last msg received from Host */
...
static int ld_phonet_tx_wait(void *data)
{
	/* Let others run while the current thread waits on bit */
	cond_resched();
	return NETDEV_TX_OK;
}

static int ld_pn_handle_tx(struct ld_phonet *ld_pn, struct sk_buff *skb)
{
	...
	if (test_and_set_bit(PHONET_SENDING, &ld_pn->state)) {
		...
		if (!netif_carrier_ok(ld_pn->dev)) {
			...
			if (skb != NULL) {
				...
				/* FIXME: 'ld_buff_len' unsafe access */
				ld_buff_len += skb->len;
			}
			return NETDEV_TX_OK;
		} else {
			wait_on_bit_lock(&ld_pn->state, PHONET_SENDING,
				ld_phonet_tx_wait, TASK_UNINTERRUPTIBLE);
		}
	}
	...
	if (!netif_carrier_ok(ld_pn->dev)) {
		...
		if (skb != NULL) {
			skb->dev->stats.tx_dropped++;
			dbg("tx_dropped = %d", skb->dev->stats.tx_dropped);
			if (in_interrupt())
				dev_kfree_skb_irq(skb);
			else
				kfree_skb(skb);
	...
	/* Drop input skb if there is no space in tty buffer */
	if (skb != NULL) {
...
{
	struct ld_phonet *ld_pn = netdev_priv(dev);
	u8 *ptr;

	/* For Ramdump debugging, Copying ISI Message
	 * RX from Modem in buffer */
	if (skb->len > sizeof(ld_phonet_tx_last_buf)) {
		printk("memcpy cancelled.\n");
	} else {
		memset(&ld_phonet_tx_last_buf,
		0, sizeof(ld_phonet_tx_last_buf));
		memcpy(ld_phonet_tx_last_buf,
		skb->data, skb->len);
	}
	...
	ptr[4] = ((unsigned short)ld_phonet_tx_request_count) >> 8;
	ptr[5] = ((unsigned short)ld_phonet_tx_request_count) & 0xFF;
	ptr[6] = skb->data[12];
	ptr[7] = skb->data[13];

	ptr = skb_put(skb, 4);
	ptr[0] = GUID_HEADER_BYTE4;
	ptr[1] = GUID_HEADER_BYTE3;
	ptr[2] = GUID_HEADER_BYTE2;
	ptr[3] = GUID_HEADER_BYTE1;

	PN_PRINTK("ld_pn_net_xmit: send skb to %s", dev->name);
	return ld_pn_handle_tx(ld_pn, skb);
}
...
static void ld_phonet_ldisc_close(struct tty_struct *tty)
{
	...
	/* Run hang up like sequence to prevent new TX and drop queued TX */
	ld_phonet_ldisc_hangup(tty);

	/* If a work item is queued, wait for it to get over */
	flush_work((struct work_struct *)ld_tty_wakeup_work);
	ld_tty_wakeup_work->ld_work_write_wakeup_tty = NULL;

	/* Clear ld_phonet object (ld_pn) reference in tty */
	tty->disc_data = NULL;

	/* ld_phonet (ld_pn) obj gets freed below when dev obj is destroyed */
	...
	while (ld_pn->n_Data_Processed < count) {
		...
		if ((count - ld_pn->n_Data_Processed) > ISI_MSG_HEADER_SIZE) {
			...
			if (ld_pn->len <= (count - ld_pn->n_Data_Processed)) {
				...
				/* For Ramdump debugging,Copying ISI Message
				 * Received from host in buffer */
				if (skb->len > sizeof(ld_phonet_rx_last_buf)) {
					printk(KERN_INFO "memcpy cancelled.\n");
				} else {
					memset(&ld_phonet_rx_last_buf,
					0,
					sizeof(ld_phonet_rx_last_buf));
					memcpy(ld_phonet_rx_last_buf,
					skb->data, skb->len);
				}
			...
			if (ld_pn->n_Remaining_Data > count) {
				...
				* Received from host in buffer */
				if(skb->len > sizeof(ld_phonet_rx_last_buf)) {
					...
					memset(&ld_phonet_rx_last_buf,
					0, sizeof(ld_phonet_rx_last_buf));
					memcpy(ld_phonet_rx_last_buf,
					skb->data,skb->len);
				}
		...
		case LD_PHONET_ISI_MSG_NO_LEN:
			...
			if ((ld_pn->len + count) >= ISI_MSG_HEADER_SIZE) {
				...
				(msglen + ISI_MSG_HEADER_SIZE)) {
					...
					/* For Ramdump debugging,Copying ISI
					* message Received from host in
					* buffer*/
					if (skb->len >
						sizeof(ld_phonet_rx_last_buf)) {
						printk("memcpy cancelled.\n");
...
static void ld_phonet_ldisc_write_wakeup(struct tty_struct *tty)
{
	...
	/* Resume TX into ld_phonet */
	netif_carrier_on(ld_pn->dev);
	netif_wake_queue(ld_pn->dev);
...
static int ld_phonet_ldisc_hangup(struct tty_struct *tty)
{
	...
	/* Do not allow new TX into ld_phonet */
	netif_carrier_off(ld_pn->dev);
	netif_stop_queue(ld_pn->dev);

	/* Flush existing TX queue */
	wait_on_bit_lock(&ld_pn->state, PHONET_SENDING,
			ld_phonet_tx_wait, TASK_UNINTERRUPTIBLE);
...
{
	printk(KERN_CRIT"LD Phonet Tx request %lu\n", \
		 ld_phonet_tx_request_count);
	printk(KERN_CRIT"LD Phonet Rx request %lu\n", \
		 ld_phonet_rx_request_count);
	printk(KERN_CRIT"LD Phonet Tx Bytes %lu\n", ld_phonet_tx_bytes);
	printk(KERN_CRIT"LD Phonet Rx Bytes %lu\n", ld_phonet_rx_bytes);
	printk(KERN_CRIT"LD Phonet TTY hangup events %lu\n", \
		ld_phonet_hangup_events);
...
static int ld_phonet_sysfs_init(void)
{
	...
	if (!ld_phonet_kobj) {
		printk(KERN_ERR"LD Sysfs Kojb failed");
		return -ENOMEM;
	}
...
static int __init ld_phonet_init(void)
{
	...
	if (ld_tty_wakeup_work) {
		...
		printk(KERN_ERR"TTY Wake up work Error\n");
		tty_unregister_ldisc(N_PHONET);
		return false;
	}
	...
	if (ld_uart_switch_work) {
		...
		printk(KERN_CRIT"UART Switch Work Failed");
	}
