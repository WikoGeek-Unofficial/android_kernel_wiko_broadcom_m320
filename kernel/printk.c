...
#ifdef CONFIG_PRINTK_COMMS
static bool printk_comms = 1;
module_param_named(comms, printk_comms, bool, S_IRUGO | S_IWUSR);
#endif
...
#ifdef CONFIG_PRINTK_CPU_ID
	VMCOREINFO_OFFSET(log, cpu_id);
#endif

#ifdef CONFIG_PRINTK_COMMS
	VMCOREINFO_OFFSET(log, comm);
#endif

#ifdef CONFIG_PRINTK_PID
	VMCOREINFO_OFFSET(log, pid);
#endif
}
#endif
...
static size_t print_comms(const char *comm, char *buf)
{
#ifdef CONFIG_PRINTK_COMMS
	if (!printk_comms)
		return 0;
	...
	return sprintf(buf, "%15.15s ", comm);
#endif
	return 0;
}
...
#ifdef CONFIG_PRINTK_COMMS
	len += print_comms(msg->comm, buf ? buf + len : NULL);
#endif
...
static size_t cont_print_text(char *text, size_t size)
{
	...
	if (cont.cons == 0 && (console_prev & LOG_NEWLINE)) {
		...
		textlen += print_comms(cont.comm, text + textlen);
		textlen += print_pid(cont.pid, text + textlen);
