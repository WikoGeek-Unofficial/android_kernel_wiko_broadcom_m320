...
static int cpufreq_stop_status = 1;

static int ctrl_cpufreq(int en);
static int ctrl_cpufreq_status(void);
...
static int ctrl_cpufreq(int en)
{
	cpufreq_stop_status = !!en;

	if (en)
		return cpufreq_del_lmt_req(&cpufreq_stop);
	else
		return cpufreq_add_lmt_req(&cpufreq_stop, "power-sysfs",
				FREQ_MAX, FREQ_MIN_LIMIT);
}

static int ctrl_cpufreq_status(void)
{
	return cpufreq_stop_status;
}
