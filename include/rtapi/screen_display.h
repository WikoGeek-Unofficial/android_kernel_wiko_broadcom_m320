...
/* DSI mode */
enum rt_standby {
	DISABLE_RT_STANDBY = 0,
	ENABLE_RT_STANDBY
};

struct screen_disp_set_rt_standby {
	void               *handle;
	unsigned short      rt_standby;
};


typedef struct {
	void               *handle;
	unsigned short      dsimode;
} screen_disp_set_dsi_mode;
...
extern int screen_display_set_dsi_mode(
screen_disp_set_dsi_mode *set_dsi_mode);

/* Set RT Standby - DISABLE_RT_STANDBY, ENABLE_RT_STANDBY */
extern int screen_display_set_rt_standby(
