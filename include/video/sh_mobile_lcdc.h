...
/* backlight info */
struct fb_lcdc_bl_info {
	const char *name;
	int max_brightness;
	int (*set_brightness)(int brightness);
};

struct fb_lcdc_overlay_cfg {
	int fourcc;
	unsigned int max_xres;
	unsigned int max_yres;
};

struct fb_lcdc_chan_cfg {
	int chan;
	int fourcc;
	...
	int num_modes;
	struct fb_lcdc_panel_cfg panel_cfg;
...
struct hw_detect_info {
	...
	unsigned int detect_gpio_val;
};

struct fb_lcdc_priv;
struct fb_lcdc_chan {
	struct fb_lcdc_priv *lcdc;
...
struct fb_lcdc_priv {
	struct device *dev;
	struct fb_lcdc_chan ch[CHAN_NUM];
	int irq;
	bool irq_enabled;
};
...
extern int sh_mobile_vsync_status(void);
