...
extern int fan5405_otg_enable_disable(int onoff);

#ifdef CONFIG_USB_OTG
int set_otg_mode(int is_host);
#endif

static inline u16 otg_read_reg(struct usb_phy *phy, u32 reg)
{
	return ioread16(phy->io_priv + reg);
}
static inline int otg_write_reg(struct usb_phy *phy, u32 val, u32 reg)
{
	 iowrite16(val, phy->io_priv + reg);
	 return 0;
}

static inline void otg_modify_reg(struct usb_phy *phy, u16 clr_mask,
				u16 set_mask, u32 reg)
{
	u16 tmp;
	tmp = ioread16(phy->io_priv + reg);
	tmp &= (~clr_mask);
	tmp |= set_mask;
	iowrite16(tmp, phy->io_priv + reg);
}
...
static inline u16 tusb1211_phy_read(struct usb_phy *phy, u32 reg)
{
	u16 monreg;
	otg_io_set_bits(phy, USB_PHYRD, USB_PHYREAD);
	while (otg_read_reg(phy, USB_PHYREAD) & USB_PHYRD)
		;
	monreg = otg_read_reg(phy, reg);
	return monreg;
}
