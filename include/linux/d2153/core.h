...
struct d2153_hwmon_platform_data {
	...
	unsigned int adc_cont_inven_len;
};
...
struct d2153_battery_platform_data {
	...
	struct d2153_ntc_temp_tb *batt_ntc_temp_tb;
	unsigned int batt_ntc_temp_tb_len;
	int flags;
	u32 recharge_voltage;
	...
	u16 poll_rate;
	u16 fake_room_temp_adc;
	enum d2153_adc_mode adc_conv_mode;
	enum d2153_adc_config_mode adc_config_mode;
	u16 chrgr_exp;
};

struct d2153_rtc_platform_data {
	/* To control alarm irq when shutdown */
	bool alarm_boot;
};
...
struct d2153_platform_data {
	...
	struct d2153_rtc_platform_data *prtc_platform;
