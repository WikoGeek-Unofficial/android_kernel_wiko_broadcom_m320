    ...
    if( smc_instance_conf->smc_shm_conf != NULL )
    {
        ...
        if( smc->smc_shm_conf != NULL )
        {
            if( smc->smc_shm_conf->shm_area_start_address != NULL )
            {
				...
				assert(smc->smc_shm_conf->size > 0);

				SMC_TRACE_PRINTF_STARTUP("physical_addr = 0x%08X",physical_addr);
				SMC_TRACE_PRINTF_STARTUP("Calling SMC_SHM_IOREMAP with smc->smc_shm_conf->shm_area_start_address = 0x%08X smc->smc_shm_conf->size =  %d",
					(uint32_t)smc->smc_shm_conf->shm_area_start_address,smc->smc_shm_conf->size);
                smc->smc_shm_conf->shm_area_start_address = SMC_SHM_IOREMAP( smc->smc_shm_conf->shm_area_start_address, smc->smc_shm_conf->size );

				if (smc->smc_shm_conf->shm_area_start_address  == NULL) {
					SMC_TRACE_PRINTF_STARTUP("SMC_SHM_IOREMAP Failed for 0x%08x",(uint32_t)smc->smc_shm_conf->shm_area_start_address);
					SMC_FREE(smc->smc_shm_conf);
					SMC_FREE(smc);
					return NULL;
				}
