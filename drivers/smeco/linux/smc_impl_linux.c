...
Version:       32   12-Jun-2014     Tero Jarkko
Status:        draft
...
struct mdm_sleep_wa_t {
	uint32_t mdm_sleep_sema;
	uint32_t mdm_sleep_ap_suggest;
	uint32_t mdm_sleep_l1_state;
	uint32_t mdm_sleep_l2_state;
	uint32_t mdm_sleep_l2_rtc;
	uint32_t mdm_sleep_gpt2_time_left;
};

#define DS_ENTER_SIGN                    0xDEE951E9
#define DS_EXIT_SIGN                     0
#define DS_SUGGEST_NO                    0x12345678
#define DS_SUGGEST_OK                    0
#define DS_SWSEMA_AP                     0x50F75EA0
#define DS_SWSEMA_CP1                    0x50F75EC1
#define DS_SWSEMA_CP2                    0x50F75EC2
#define SDRAM_MDM_SLEEP_ENTRY_START_ADDR 0x758FFE84
...
static struct mdm_sleep_wa_t *sdram_mdm_sleep_entry;

//static struct wake_lock* wakelock    = NULL;
static inline void get_sdram_mdm_sleep_entry(void)
{
	sdram_mdm_sleep_entry = (struct mdm_sleep_wa_t *)ioremap_nocache(
		SDRAM_MDM_SLEEP_ENTRY_START_ADDR,
		sizeof(struct mdm_sleep_wa_t));
}

static inline void get_mdm_sleep_semaphore(void)
{
	uint32_t i;

	i = sdram_mdm_sleep_entry->mdm_sleep_sema;
	if ((i != DS_SWSEMA_AP) && (i != DS_SWSEMA_CP1) && (i != DS_SWSEMA_CP2))
		sdram_mdm_sleep_entry->mdm_sleep_sema = 0;
	for (i = 0; i < 1000; i++) {
		if (sdram_mdm_sleep_entry->mdm_sleep_sema == DS_SWSEMA_AP)
			return;
		if (sdram_mdm_sleep_entry->mdm_sleep_sema == 0)
			sdram_mdm_sleep_entry->mdm_sleep_sema =	DS_SWSEMA_AP;
		udelay(2);
	}
	BUG_ON(1);
}

static inline void release_mdm_sleep_semaphore(void)
{
	sdram_mdm_sleep_entry->mdm_sleep_sema = 0;
}

static inline void smc_host_access_wakeup(smc_lock_t *spinlock,
					uint32_t timeout)
{
	uint32_t timer = 0;
	...
	if (spinlock != NULL)
		SMC_LOCK_IRQ(spinlock);

	if (sdram_mdm_sleep_entry != NULL) {
		get_mdm_sleep_semaphore();
		sdram_mdm_sleep_entry->mdm_sleep_ap_suggest = DS_SUGGEST_NO;
		if (sdram_mdm_sleep_entry->mdm_sleep_l1_state == DS_ENTER_SIGN
			&& sdram_mdm_sleep_entry->mdm_sleep_l2_state ==
			DS_ENTER_SIGN) {
			release_mdm_sleep_semaphore();
			mdelay(1);
		} else
			release_mdm_sleep_semaphore();
	}

	__raw_writel(SMC_WPMCIF_EPMU_ACC_CR_MODEM_ACCESS_REQ,
		SMC_WPMCIF_EPMU_ACC_CR);
	(void)__raw_readl(SMC_WPMCIF_EPMU_ACC_CR);
	while (SMC_WPMCIF_EPMU_ACC_CR_MODEM_ACCESS_OK !=
		__raw_readl(SMC_WPMCIF_EPMU_ACC_CR)) {
...
static inline void smc_host_access_sleep(smc_lock_t *spinlock)
{
	if (sdram_mdm_sleep_entry != NULL) {
		get_mdm_sleep_semaphore();
		sdram_mdm_sleep_entry->mdm_sleep_ap_suggest = DS_SUGGEST_OK;
		release_mdm_sleep_semaphore();
	}

	__raw_writel(SMC_WPMCIF_EPMU_ACC_CR_MODEM_SLEEP_REQ,
		SMC_WPMCIF_EPMU_ACC_CR);
	(void)__raw_readl(SMC_WPMCIF_EPMU_ACC_CR);
	if (spinlock != NULL)
		SMC_UNLOCK_IRQ(spinlock);
}
...
uint16_t smc_asic_version_get(void)
{
    ...
    if (shmobile_is_u2b())
	    return 0x23;/* Hardcoding the value.
			   Loading Modem images check for this version */
    else
	    return cpu_version;
}
