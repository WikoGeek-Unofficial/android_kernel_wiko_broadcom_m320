...
/* To accomodate 4-byte GUID when sent to PC (see ld-phonet) */
#define SMC_SKB_EXTRA_TAIL_ROOM 4
...
static void smc_conf_update(phys_addr_t shm_start_address);
...
smc_device_driver_config dev_config_l2mux =
{
	...
	.smc_conf_update	 = smc_conf_update,
...
static void smc_conf_update(phys_addr_t shm_start_address)
	{
	smc_instance_conf_t *smc_instance_conf;
	...
	smc_cpu_name = smc_instance_conf_name_get_from_list(
		smc_instance_conf_l2mux, SMC_CONF_COUNT_L2MUX,
		SMC_CONFIG_USER_L2MUX, TRUE, version_major, version_minor);
	smc_instance_conf = smc_instance_conf_get_l2mux(
		SMC_CONFIG_USER_L2MUX, smc_cpu_name);
	smc_instance_conf->shm_start_address = shm_start_address +
		SMC_CONF_L2MUX_SHM_START_OFFSET_ES20;
	}
/*
...
static smc_conf_t* smc_device_create_conf_l2mux(char* device_name)
{
	...
	smc_instance_conf->shm_start_address =
			get_mem_resource("SDRAM_CP_SHARED")->start +
			SMC_CONF_L2MUX_SHM_START_OFFSET_ES20;
    ...
    if( data != 0x00000000 )
    {
        ...
        if( skb_receive_data != NULL )
        {
			...
			skb = netdev_alloc_skb(device, data_length +
					SMC_L2MUX_HEADER_SIZE +
					SMC_SKB_EXTRA_TAIL_ROOM);
        ...
        else
        {
            ...
            if( unlikely(!skb) )
            {
				...
				skb = __netdev_alloc_skb(device,
					...
					SMC_L2MUX_HEADER_SIZE +
					SMC_SKB_EXTRA_TAIL_ROOM,
					GFP_KERNEL|__GFP_HIGH|__GFP_HIGHMEM);
  #endif
            }
