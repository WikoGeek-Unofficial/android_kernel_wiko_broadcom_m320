...
#ifdef CONFIG_MOBICORE_API
	union fc_generic comaentry;

	memset(&comaentry, 0, sizeof(comaentry));

	comaentry.as_in.cmd = MC_FC_SECONDARY_CORE_SLEEP;
	comaentry.as_in.param[0] = mode;
	comaentry.as_in.param[1] = wakeup_address;
	comaentry.as_in.param[2] = 0;

	_smc(&comaentry);

	sec_hal_status = comaentry.as_out.ret;

	if (SEC_HAL_RES_OK != sec_hal_status) {
		printk_ratelimited(KERN_WARNING
			"[%s] returned a fail!! serv == %d\n",
			__func__, sec_hal_status);
...
#ifdef CONFIG_MOBICORE_API
	union fc_generic power_off;

	memset(&power_off, 0, sizeof(power_off));

	power_off.as_in.cmd = MC_FC_POWER_OFF;
	power_off.as_in.param[0] = 0;
	power_off.as_in.param[1] = 0;
	power_off.as_in.param[2] = 0;

	_smc(&power_off);

	sec_hal_status = power_off.as_out.ret;
...
#ifdef CONFIG_MOBICORE_API
	union fc_generic a3sp_state;

	memset(&a3sp_state, 0, sizeof(a3sp_state));

	a3sp_state.as_in.cmd = MC_FC_A3SP_CONTROL;
	a3sp_state.as_in.param[0] = A3SP_PUBLIC_REQ;
	a3sp_state.as_in.param[1] = state;
	a3sp_state.as_in.param[2] = 0;

	_smc(&a3sp_state);

	sec_hal_status = a3sp_state.as_out.ret;
...
#ifdef CONFIG_MOBICORE_API
	union fc_generic sl2spinlock;

	memset(&sl2spinlock, 0, sizeof(sl2spinlock));
	sl2spinlock.as_in.cmd = MC_FC_BCM_L2_CACHE_CONTROL;

#ifdef SEC_HAL_TICKET_SPINLOCK_SUPPORT
	/* For ARM kernel version 3.6 and onwards */
	sl2spinlock.as_in.param[0]=L2_SPINLOCK_TYPE_TICKET;
#else
	/* For ARM kernel version earlier than 3.6 */
	sl2spinlock.as_in.param[0]=L2_SPINLOCK_TYPE_DEFAULT;
#endif /* SEC_HAL_TICKET_SPINLOCK_SUPPORT*/

	sl2spinlock.as_in.param[1]=spinlock_phys_addr;
	_smc(&sl2spinlock);

	sec_hal_status = sl2spinlock.as_out.ret;
