...
#define LOCAL_L1L2_CACHE_CLEAN_AND_INV

#ifndef CONFIG_MOBICORE_API
/* empty unlock code for simlock */
static const char *k_sec_hal_simlock_empty_code = "EMPTY";
#endif
...
uint32_t sec_hal_rt_key_info_get(sec_hal_key_info_t *user_key_info_out)
{
    ...
    if (NULL == in_msg || NULL == out_msg) {
	  ...
	  sec_msg_status = _sec_msg_param_read((cb_memcpy) LOCAL_CPY_TO_CLIENT_FUNCPTR,
	...
	uint32_t *obj_id)
{
	...
	do {
		...
		if (copy_from_user(kcert, (const void __user *)cert,
				cert_size)) {
	...
	uint32_t *id_ptr)
{
	...
	do {
		...
		if (copy_from_user(kdata, (const void __user *)data,
				data_size)) {
    ...
    if (NULL == in_msg || NULL == out_msg) {
        ...
        if (SEC_ROM_RET_OK != ssa_disp_status ||
            ...
            sec_msg_status = _sec_msg_param_read(
				(cb_memcpy) LOCAL_CPY_TO_CLIENT_FUNCPTR,
                    &out_handle,
                    user_imei_out,
                    SEC_HAL_MAX_IMEI_SIZE);
            if (SEC_MSG_STATUS_OK != sec_msg_status) {
	...
	uint32_t user_auth_data_size_in)
{
	...
	do {
		...
		if (copy_from_user(kdata,
			(const void __user *)user_input_data_in,
			user_input_data_size_in)) {
			SEC_HAL_TRACE("kmalloc failed, msgs not sent!");
			ret = SEC_HAL_RES_FAIL;
			break;
...
/* ****************************************************************************
** Function name      : sec_hal_device_crypt
** Description        : This function is used to encrypt a length of given
**                      continuous input data using device security services.
**
** Parameters         : IN/--- uint32_t operation
**                      IN/--- uint32_t in_length
**                      IN/--- uint8_t *in_data_ptr
**                      IN/--- uint8_t *iv_in_ptr,
**                      IN/--- uint32_t reserved_out_length
**                      --/OUT uint32_t *out_length
**                      --/OUT uint8_t *out_data_ptr
** Return value       : uint32
**                      ==0 operation successful
**                      failure otherwise.
** ***************************************************************************/
uint32_t sec_hal_device_crypt(
	uint32_t operation,
	uint32_t in_length,
	uint8_t *in_data_ptr,
	uint8_t *iv_in_ptr,
	uint32_t reserved_out_length,
	uint32_t *out_length,
	uint8_t *out_data_ptr)
{
	...
	uint32_t sec_msg_st = SEC_MSG_STATUS_OK;
	uint32_t ssa_disp_st = 0x00;
	sec_serv_status_t sec_serv_st = 0x00;
	uint16_t msg_data_size;
	sec_msg_t *in_msg = 0x0;
	sec_msg_t *out_msg = 0x0;
	sec_msg_handle_t in_handle;
	sec_msg_handle_t out_handle;
	void *k_in_data_ptr = NULL;
	void *k_out_data_ptr = NULL;

	SEC_HAL_TRACE_ENTRY();

	/* Alloc */
	msg_data_size = sec_msg_param_size(sizeof(uint32_t))*5
			+ sec_msg_param_size(SEC_IV_LENGTH);
	in_msg = sec_msg_alloc(&in_handle, msg_data_size,
			SEC_MSG_OBJECT_ID_NONE, 0, SEC_HAL_MSG_BYTE_ORDER);
	msg_data_size = sec_msg_param_size(sizeof(sec_serv_status_t))
			+ sec_msg_param_size(sizeof(uint32_t));
	...
	do {
		...
		if (sec_msg_param_write32(&in_handle,
			operation,
			SEC_MSG_PARAM_ID_NONE) != SEC_MSG_STATUS_OK) {
			SEC_HAL_TRACE("operation write error, aborting!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}
		if (sec_msg_param_write32(&in_handle,
			in_length,
			SEC_MSG_PARAM_ID_NONE) != SEC_MSG_STATUS_OK) {
			SEC_HAL_TRACE("in_length write error, aborting!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}
		if (sec_msg_param_write32(&in_handle,
			reserved_out_length,
			SEC_MSG_PARAM_ID_NONE) != SEC_MSG_STATUS_OK) {
			SEC_HAL_TRACE("reserved_out_length write error!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}
		if (sec_msg_param_write(&in_handle,
			iv_in_ptr, SEC_IV_LENGTH,
			SEC_MSG_PARAM_ID_NONE) != SEC_MSG_STATUS_OK) {
			SEC_HAL_TRACE("iv_in_ptr write error, aborting!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}

		k_in_data_ptr = kmalloc(in_length, GFP_KERNEL);
		if (k_in_data_ptr == NULL) {
			SEC_HAL_TRACE("kmalloc failed, msgs not sent!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}
		if (copy_from_user(k_in_data_ptr,
				   (const void __user *)in_data_ptr,
				   in_length)) {
			SEC_HAL_TRACE("copy failed, msgs not sent!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		...
		if (sec_msg_param_write32(&in_handle,
			(uint32_t)virt_to_phys(k_in_data_ptr),
			SEC_MSG_PARAM_ID_NONE) != SEC_MSG_STATUS_OK) {
			SEC_HAL_TRACE("in_data_ptr write error, abort!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}

		k_out_data_ptr = kmalloc(reserved_out_length, GFP_KERNEL);
		if (k_out_data_ptr == NULL) {
			SEC_HAL_TRACE("kmalloc failed, msgs not sent!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}
		if (sec_msg_param_write32(&in_handle,
			(uint32_t)virt_to_phys(k_out_data_ptr),
			SEC_MSG_PARAM_ID_NONE) != SEC_MSG_STATUS_OK) {
			SEC_HAL_TRACE("out_data_ptr write error, abort!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}
		LOCAL_WMB();
	    ...
	    /* Dispatcher */
		ssa_disp_st = LOCAL_DISP(SEC_SERV_DEVICE_CRYPT,
				LOCAL_DEFAULT_DISP_FLAGS,
				LOCAL_DEFAULT_DISP_SPARE_PARAM,
				SEC_HAL_MEM_VIR2PHY_FUNC(out_msg),
				SEC_HAL_MEM_VIR2PHY_FUNC(in_msg));
				LOCAL_L1L2_CACHE_CLEAN_AND_INV;
		...
		sec_msg_st = sec_msg_param_read32(&out_handle, out_length);

		if (copy_to_user((void __user *)out_data_ptr, k_out_data_ptr,
				*out_length)) {
			SEC_HAL_TRACE("copy failed, output not sent!");
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}
		if (SEC_MSG_STATUS_OK != sec_msg_st) {
			SEC_HAL_TRACE(
				"failed to read out_length! msg==%d\n",
				sec_msg_st);
			sec_hal_st = SEC_HAL_RES_FAIL;
			break;
		}
...
long sec_hal_rt_ioctl(unsigned int cmd, void **data, sd_ioctl_params_t *param)
{
	...
	case SD_DEV_CRYPT:
		{
		void *iv_usr = (void *)param->param3;

		if (iv_usr &&
			copy_from_user(iv,
				(const void *)iv_usr, SEC_IV_LENGTH)) {
			return SEC_HAL_RES_FAIL;
			}
		rv = sec_hal_device_crypt(
				param->param0 /* IN */,
				param->param1 /* IN */,
				(uint8_t *)param->param2 /* IN */,
				iv /* IN */,
				param->param4 /* IN */,
				(uint32_t *)param->param5 /* OUT */,
				(uint8_t *)param->reserved1 /* OUT */);
		}
		break;
	default:
		rv = -EPERM;
