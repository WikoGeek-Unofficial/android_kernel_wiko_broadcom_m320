...
int sec_hal_rpc_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos);
	...
	unsigned long arg)
{
	...
	case SD_DEV_CRYPT:
		...
		break;
	case SD_PUBLIC_ID:
		ret = copy_to_user((void __user *)input.param0,
			g_pub_id.public_id,
...
static const struct file_operations k_sec_hal_fops = {
    .owner = THIS_MODULE,
