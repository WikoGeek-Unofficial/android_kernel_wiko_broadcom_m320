...
#define MAX_EVT_LOG_ENTRIES	(255)
...
struct {
	int evt;
	int data;
} spa_evt_log[MAX_EVT_LOG_ENTRIES];
...
static int spa_get_full_charge(struct spa_power_desc *spa_power_iter);
		...
		case SPA_POWER_PROP_EVT_LOG:
			{
				...
				count += scnprintf(buf + count, PAGE_SIZE - count, "evt_index=%d\n", spa_evt_idx);

				for (i = 0; i < MAX_EVT_LOG_ENTRIES; i++) {
					count += scnprintf(buf + count, PAGE_SIZE - count, "evt_log[%d]=%d,%d\n", i, \
								spa_evt_log[i].evt, spa_evt_log[i].data);
				}
...
static struct device_attribute ss_batt_ext_attrs[]=
{
	...
	__ATTR(batt_reset_soc, S_IWUSR, NULL, ss_batt_ext_attrs_store),
	__ATTR(batt_vol_aver, S_IRUGO, ss_batt_ext_attrs_show, NULL),
	__ATTR(batt_ntc_disable, S_IRUGO | S_IWUSR,
		ss_batt_ext_attrs_show, ss_batt_ext_attrs_store),
		/*for CTIA test*/
#if defined(CONFIG_TARGET_LOCALE_USA)
...
static int spa_set_charge(struct spa_power_desc *spa_power_iter, unsigned int act)
{
	...
	else if(act == SPA_CMD_USB_UNCONFIG)
	{
		spa_power_iter->charger_info.charging_current = 100;

		cancel_delayed_work_sync(&spa_power_iter->fast_charging_work);
		queue_delayed_work(spa_power_iter->spa_workqueue,
			&spa_power_iter->fast_charging_work, 0);
	}
	else if (act == SPA_CMD_USB_CONFIG)
	{
		spa_power_iter->charger_info.charging_current = 500;

		cancel_delayed_work_sync(&spa_power_iter->fast_charging_work);
		queue_delayed_work(spa_power_iter->spa_workqueue,
			&spa_power_iter->fast_charging_work, 0);
	}
	else if(act == SPA_CMD_CHARGE)
	{
		// 1. Charging current
...
static int spa_get_full_charge(struct spa_power_desc *spa_power_iter)
{
	...
	ps->get_property(ps, POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN, &value);

	spa_power_iter->batt_info.full_capacity = value.intval;

	pr_spa_dbg(LEVEL2, "%s : full charge, %d\n", __func__, value.intval);

	pr_spa_dbg(LEVEL4, "%s : leave\n", __func__);
	return spa_power_iter->batt_info.full_capacity;
}
...
static int spa_do_status(struct spa_power_desc *spa_power_iter, unsigned char machine, unsigned int phase, unsigned int status)
{
	...
	if (machine == SPA_MACHINE_NORMAL) {
		...
		} else if (phase == POWER_SUPPLY_STATUS_CHARGING) /* CHARGING */
		{
			...
			switch (status) {
			case SPA_STATUS_USB_SUSPEND:
				spa_set_charge(spa_power_iter,
						SPA_CMD_DISCHARGE);
				spa_power_iter->charging_status.status=
					SPA_STATUS_USB_SUSPEND;
				break;
			case SPA_STATUS_USB_UNCONFIG:
				spa_set_charge(spa_power_iter,
						SPA_CMD_USB_UNCONFIG);
				spa_power_iter->charging_status.status=
					SPA_STATUS_USB_UNCONFIG;
				break;
			case SPA_STATUS_USB_CONFIG:
				spa_set_charge(spa_power_iter,
						SPA_CMD_USB_CONFIG);
				spa_power_iter->charging_status.status=
...
static void spa_update_batt_info(struct spa_power_desc *spa_power_iter, unsigned int prop)
{
	...
	switch(prop)
	{
		...
		case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
			value.intval = spa_power_iter->batt_info.full_capacity;
...
static void spa_batt_work(struct work_struct *work)
{
	...
	if (spa_power_iter->dbg_simul != 1) {
		...
		spa_power_iter->batt_info.full_capacity = spa_get_full_charge(spa_power_iter);
	}
	...
	spa_update_power_supply_battery(spa_power_iter, POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN);
...
int spa_event_handler(int evt, void *data)
{
	...
	spa_evt_log[(spa_evt_idx++) % MAX_EVT_LOG_ENTRIES].evt=evt;
	spa_evt_log[(spa_evt_idx) % MAX_EVT_LOG_ENTRIES].data = (int)data;
	...
	switch(evt)
	{
		...
		case SPA_EVT_CAPACITY:
			...
			break;
		case SPA_EVT_USB_SUSPEND:
			spa_do_status(spa_power_iter, SPA_MACHINE_NORMAL,
				POWER_SUPPLY_STATUS_CHARGING,
				SPA_STATUS_USB_SUSPEND);
			break;
		case SPA_EVT_USB_UNCONFIG:
			spa_do_status(spa_power_iter, SPA_MACHINE_NORMAL,
				POWER_SUPPLY_STATUS_CHARGING,
				SPA_STATUS_USB_UNCONFIG);
			break;
		case SPA_EVT_USB_CONFIG:
			spa_do_status(spa_power_iter, SPA_MACHINE_NORMAL,
				POWER_SUPPLY_STATUS_CHARGING,
				SPA_STATUS_USB_CONFIG);
			break;
