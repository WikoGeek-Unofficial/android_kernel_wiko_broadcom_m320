...
#define SPA_AGENT_DBG_LEVEL_INTERNAL SPA_AGENT_DBG_LEVEL0
				...
				union power_supply_propval *propval)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		if (spa_agent_fn[SPA_AGENT_GET_CURRENT].fn.get_spa_current)
			/* eoc check */
			propval->intval = spa_agent_fn[SPA_AGENT_GET_CURRENT].fn.get_spa_current(1);
		else
			ret = -ENODATA;
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		pr_spa_agent_dbg(LEVEL3, "%s : agent id = %d\n",
					__func__, SPA_AGENT_GET_FULL_CHARGE);
		if (spa_agent_fn[SPA_AGENT_GET_FULL_CHARGE].fn.get_full_charge)
			propval->intval = spa_agent_fn[SPA_AGENT_GET_FULL_CHARGE].fn.get_full_charge();
		else
			ret = -ENODATA;
		break;
	default:
