...
#define custom_t_SEC_YEAR_BASE 			14 
#define custom_t_SEC_MONTH_BASE 			06  
#define custom_t_SEC_DAY_BASE	 			01  
#endif
...
static void d2153_rtc_alarm_notify(struct work_struct *work)
{
	struct d2153_rtc *dlg_rtc =
		container_of(work, struct d2153_rtc, work);

	kobject_uevent(&dlg_rtc->rtc->dev.kobj, KOBJ_CHANGE);
}
...
static irqreturn_t d2153_rtc_timer_alarm_handler(int irq, void *data)
{
	...
	struct d2153_rtc_platform_data *prtc_data = d2153->pdata->prtc_platform;

	rtc_update_irq(rtc, 1, RTC_AF | RTC_IRQF);
	/* Notify app(off_charge) if alarm_boot is supported */
	if (prtc_data && prtc_data->alarm_boot)
		schedule_work(&d2153->rtc.work);
...
#ifdef custom_t_CUSTOM
	current_rtc_time.tm_year += custom_t_SEC_YEAR_BASE;
	current_rtc_time.tm_mon  = custom_t_SEC_MONTH_BASE;
	current_rtc_time.tm_mday = custom_t_SEC_DAY_BASE;
	dlg_info("custom_t:current_rtc_time is %d %d %d\n",custom_t_SEC_YEAR_BASE,custom_t_SEC_MONTH_BASE,custom_t_SEC_DAY_BASE);
#else
...
static int d2153_rtc_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&dlg_rtc->work, d2153_rtc_alarm_notify);
