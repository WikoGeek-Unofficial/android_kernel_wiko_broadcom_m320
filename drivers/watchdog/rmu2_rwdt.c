...
int rmu2_rwdt_cntclear(void)
{
	...
	if (0U == wrflg) {
		...
		/*
		 * PREEMPT_BITS -> 8, we are currently only interested in logging preempt_count
...
{
	unsigned long flags;
	static DEFINE_SPINLOCK(workfn_lock);
	...
	*/
	...
	spin_lock_irqsave(&workfn_lock, flags);
	...
	spin_unlock_irqrestore(&workfn_lock, flags);
}
...
{
	rmu2_touch_softlockup_watchdog();
}
