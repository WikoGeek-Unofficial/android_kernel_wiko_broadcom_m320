...
atomic_t is_psensor_locked;
static int als_ps_int;
static int als_ps_gpio_inr;

#ifdef RUNTIME_PM
static struct regulator *prxamlite_regltr_3v;
static struct regulator *prxamlite_regltr_18v;
#endif
...
#ifdef RUNTIME_PM
	if (!atomic_read(&is_psensor_locked))
		tmd2771_power_on_off(1);
#endif
	ret = i2c_smbus_write_byte(taos_datap->client,
					TAOS_TRITON_CMD_REG |
					TAOS_TRITON_CNTRL);
...
static int __taos_early_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (atomic_read(&is_psensor_locked)) {
		pr_taos(DEBUG, "TAOS: skip suspend sensor-locked\n");
		mutex_unlock(&taos_datap->mutex);
		return 0;
	}
		      ...
		      const struct i2c_device_id *idp)
{
	...
	ret = gpio_request(als_ps_gpio_inr, "NULL");
	if (ret < 0) {
...
#ifdef RUNTIME_PM
	if (prxamlite_regltr_18v) {
		tmd2771_cs_power_on_off(0);
		regulator_put(prxamlite_regltr_18v);
	}
err_regulator_request:
	if (prxamlite_regltr_3v) {
		tmd2771_power_on_off(0);
...
static long taos_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TAOS_IOCTL_PROX_ON:
		...
		if (!atomic_read(&is_psensor_locked)) {
			...
			atomic_set(&is_psensor_locked, 1);
		}
		ret = i2c_smbus_write_byte_data(taos_datap->client,
						TAOS_TRITON_CMD_REG | 0x01,
						taos_cfgp->prox_int_time);
	...
	case TAOS_IOCTL_PROX_OFF:
		...
		if (atomic_read(&is_psensor_locked)) {
			...
			atomic_set(&is_psensor_locked, 0);
		}
		break;
