...
static const struct of_device_id k3dh_of_match[] = {
	{.compatible = "st,k2hh", "st,k3dh"},
	{},
};
MODULE_DEVICE_TABLE(of, k3dh_of_match);
...
static int k3dh_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct k3dh_platform_data *platform_data = NULL;
	int err=0;
    	int ii = 0;
        int tempvalue;        
        unsigned char acc_data[2] = {0};
	struct device_node *np;
	const struct of_device_id *match = of_match_device(k3dh_of_match,
			&client->dev);
	...
	if (!match && client->dev.platform_data) {
		...
		k3dh->pdata = client->dev.platform_data;
		...
		if (of_property_read_u8_array(np, "orientation",
			platform_data->orientation,
				sizeof(platform_data->orientation))) {
			dev_err(&client->dev, "failed to retrive orientation");
			goto exit;
		}
		k3dh->pdata = platform_data;
	}
...
static struct i2c_driver k3dh_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "k3dh",
		.of_match_table = of_match_ptr(k3dh_of_match),
#if !defined(CONFIG_SENSORS_HSCDTD006A)  && !defined(CONFIG_SENSORS_HSCDTD008A)
                .pm = &k3dh_pm_ops,
#endif
	},
...
static int __init k3dh_init(void)
{
	...
	/* regulator init */
    	...
    	if (IS_ERR(sensor1v8_regulator)){
    	    printk(KERN_ERR "[K3DH] can not get prox_regulator (SENSOR_1.8V) \n");
    	} else {
