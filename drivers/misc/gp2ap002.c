...
static void gp2ap002_led_onoff(bool onoff)
{
	...
	if (onoff) {
		...
		if (IS_ERR(led_regulator)) {
			pr_err("[GP2A] can not get prox_regulator (SENSOR_LED_3.0V)\n");
		} else {
			ret = regulator_set_voltage(led_regulator,
					3300000, 3300000);
			pr_info("[GP2A] regulator_set_voltage : %d\n", ret);
			ret = regulator_enable(led_regulator);
			pr_info("[GP2A] regulator_enable : %d\n", ret);
		...
		pr_info("[GP2A] regulator_disable : %d\n", ret);
		regulator_put(led_regulator);
	}
}
...
static int gp2a_prox_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	int ret =0;
	u8 reg_value;
	struct gp2ap002_platform_data *platform_data = NULL;
	struct device_node *np;
	const struct of_device_id *match = of_match_device(gp2ap002_of_match,
			&client->dev);

	pr_info("[GP2A] %s start\n", __func__);
	...
	if (!match && client->dev.platform_data) {
		...
		platform_data->irq_gpio = of_get_named_gpio(np,
				"PROXI_INT", 0);
	}

	gp2a_data->power_on = gp2ap002_power_onoff;
	gp2a_data->led_on = gp2ap002_led_onoff;
	...
	gpio_pull_up_port(gp2a_data->irq_gpio);
	...
	ret = request_threaded_irq(gp2a_data->irq, NULL, gp2a_irq_handler,
			IRQF_TRIGGER_FALLING | IRQF_ONESHOT | IRQF_NO_SUSPEND, "proximity_int", gp2a_data);
	if (unlikely(ret < 0)) {
