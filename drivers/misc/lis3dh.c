...
static int lis3dh_acc_read_device_id(struct lis3dh_acc_data *acc, u8 *id)
{
	int err;
	u8 buf[7];
	int i;
...
static int lis3dh_acc_hw_init(struct lis3dh_acc_data *acc)
{
	...
	for (i = 0; i < 5; i++) {
		err = lis3dh_acc_read_device_id(acc, &did);
		if (err == -EIO)
			goto error_firstread;
		if (did == WHOAMI_LIS3DH_ACC)
			break;
		else if (i == 4)
...
static void lis3dh_acc_device_power_off(struct lis3dh_acc_data *acc)
{
	...
	if (acc->hw_initialized) {
		if (acc->pdata->gpio_int1 != -1)
			disable_irq_nosync(acc->irq1);
		if (acc->pdata->gpio_int2 != -1)
...
{
	input_report_abs(acc->input_dev, ABS_X, xyz[0] - lis3dh_offset[0]);
	input_report_abs(acc->input_dev, ABS_Y, xyz[1] - lis3dh_offset[1]);
	input_report_abs(acc->input_dev, ABS_Z, xyz[2] - lis3dh_offset[2]);
				...
				unsigned long arg)
{
	...
	break;
	case LIS3DH_ACC_IOCTL_GET_OFFSET:
		if (copy_to_user(argp, lis3dh_offset, sizeof(lis3dh_offset))) {
			pr_err("%s: %s error in copy offset to user\n",
			LIS3DH_ACC_DEV_NAME, __func__);
			return -EINVAL;
		}
		break;
	case LIS3DH_ACC_IOCTL_SET_OFFSET:
		if (copy_from_user(lis3dh_offset, argp,
			sizeof(lis3dh_offset))) {
			pr_err("%s: set offset fail\n", __func__);
			return -EFAULT;
		}
		break;
	default:
