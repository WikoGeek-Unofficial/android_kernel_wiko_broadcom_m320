...
static int add_to_workqueue(struct tpu_work_arg *work_arg)
{
	...
	case TPU_OPEN:
		INIT_WORK_ONSTACK(&work_arg->work, comp_tpu_open);
		break;
	case TPU_ENABLE:
		INIT_WORK_ONSTACK(&work_arg->work, comp_tpu_enable);
		break;
	case TPU_CLOSE:
		INIT_WORK_ONSTACK(&work_arg->work, comp_tpu_close);
		break;
	case TPU_RESUME:
		INIT_WORK_ONSTACK(&work_arg->work, handle_tpu_resume);
		break;
	case TPU_SUSPEND:
		INIT_WORK_ONSTACK(&work_arg->work, handle_tpu_suspend);
		break;
	default:
		return -EINVAL;
	}
...
static int handle_tpu_enable(void *param)
{
	...
	*/
	...
	/*
	 * This driver previously had individual pin control, for each channel,
	 * but this is hard to achieve with pinctrl, which wants per-driver
	 * states. So we've simplified down, and just use whole-driver
	 * default and sleep states now. The pins will be already selected.
	 */
