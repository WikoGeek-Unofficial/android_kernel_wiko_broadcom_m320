...
#define KXTIK_RUNTIME_PM
#if KXTIK_DEBUG
#define FUNCDBG(format, arg...)   pr_info("KXTIK " format , ## arg)
#else
...
#ifdef KXTIK_RUNTIME_PM
static struct regulator *kxtik_regltr_3v;
static struct regulator *kxtik_regltr_18v;
static int kxtik_power_on_off(bool flag);
static int kxtik_cs_power_on_off(bool flag);
#endif
		...
		if (atomic_read(&tik->acc_enable_resume) > 1) {
			while (loop) {
				if (mutex_trylock(&tik->data_mutex) == 0) {
					pr_err("acceleration_data:error mutex try lock");
					return;
				}
...
static int kxtik_verify(struct kxtik_data *tik)
{
	...
	if (retval < 0)
		pr_err("error reading WHO_AM_I register!\n");
	else {
...
#ifdef KXTIK_SUSPEND_PM
	kxtik_power_on_off(0);
	kxtik_cs_power_on_off(0);
#endif
	return;
}
...
#ifdef KXTIK_SUSPEND_PM
	kxtik_cs_power_on_off(1);
	kxtik_power_on_off(1);
#endif
...
static int kxtik_power_on_off(bool flag)
{
	int ret;
	if (!kxtik_regltr_3v) {
		pr_err("kxtik_regltr_3v is unavailable\n");
		return -1;
	}
	if ((flag == 1)) {
		pr_info("\n LDO on %s ", __func__);
		ret = regulator_enable(kxtik_regltr_3v);
		if (ret)
			pr_err("kxtik Regulator-enable failed\n");
	} else if ((flag == 0)) {
		pr_info("\n LDO off %s ", __func__);
		ret = regulator_disable(kxtik_regltr_3v);
		if (ret)
			pr_err("kxtik 3v Regulator disable failed\n");
	}
	return 0;
}
...
static int kxtik_cs_power_on_off(bool flag)
{
	int ret;
	if (!kxtik_regltr_18v) {
		pr_err("kxtik_regltr_18v is unavailable\n");
		return -1;
	}
	if ((flag == 1)) {
		pr_info("\n LDO on %s ", __func__);
		ret = regulator_enable(kxtik_regltr_18v);
		if (ret)
			pr_err("kxtik 1v8 Regulator enable failed\n");
	} else if ((flag == 0)) {
		pr_info("\n LDO off %s ", __func__);
		ret = regulator_disable(kxtik_regltr_18v);
		if (ret)
			pr_info("kxtik 1v8 Regulator disable failed\n");
	}
	return 0;
}
#endif
				 ...
				 const struct i2c_device_id *id)
{
	...
	pr_info("kxtik_probe\n");
#ifdef KXTIK_RUNTIME_PM
	pr_info("%s: kxtik setting up regulators\n", __func__);
	kxtik_regltr_3v = regulator_get(NULL, "sensor_3v");
	if (IS_ERR_OR_NULL(kxtik_regltr_3v)) {
		pr_err("%s: kitik Failed to get regulator\n", __func__);
		return -EBUSY;
	}
	regulator_set_voltage(kxtik_regltr_3v, 3000000, 3000000);
	err = regulator_enable(kxtik_regltr_3v);
	if (err)
		pr_err("kxtik Regulator Enable Failed\n");
	kxtik_regltr_18v = regulator_get(NULL, "sensor_1v8");
	if (IS_ERR_OR_NULL(kxtik_regltr_18v)) {
		pr_err("%s:kxtik 18v Failed to get regulator\n",
			__func__);
		err = -EBUSY;
		goto err_pwr;
	}
	regulator_set_voltage(kxtik_regltr_18v, 1800000, 1800000);
	err = regulator_enable(kxtik_regltr_18v);
	if (err)
		pr_err("kxtik 1v8 Regulator enable failed\n");
#endif
	...
	if (unlikely(!tik->irq_workqueue)) {
		pr_err("create KXTIK Workqueue failed\n");
	...
	if (client->irq) {
		pr_err("__kxtik is in irq mode__\n");
		...
		pr_err("__kxtik is in polling mode__\n");
...
err_free_mem:
	...
	if (kxtik_regltr_18v) {
		kxtik_power_on_off(0);
		regulator_put(kxtik_regltr_18v);
	}

err_pwr:
	if (kxtik_regltr_3v) {
		kxtik_power_on_off(0);
		regulator_put(kxtik_regltr_3v);
	}
#endif
	return err;
}
...
static void kxtik_shutdown(struct i2c_client *client)
{
	...
	err = i2c_smbus_write_byte_data(tik->client, CTRL_REG1, 0);
	if (err < 0)
		pr_err("kxtik failed to shudown\n");
