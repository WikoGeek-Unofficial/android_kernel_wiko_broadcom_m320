				...
				struct bcm_bt_lpm_entry_struct *priv)
{
	...
	irq_set_irq_wake(priv->plpm->host_irq, 0);
...
static int bcm_bt_lpm_suspend(struct platform_device *pdev, pm_message_t state)
{
	if (priv_g == NULL)
		return 0;

	if (priv_g->bt_lpm_tty != NULL) {
		tty_throttle(priv_g->bt_lpm_tty);
		pr_debug("%s BLUETOOTH:Calling to flow OFF RTS.\n", __func__);
	}
	return 0;
}

static int bcm_bt_lpm_resume(struct platform_device *pdev)
{
	if (priv_g == NULL)
		return 0;

	if (priv_g->bt_lpm_tty != NULL) {
		tty_unthrottle(priv_g->bt_lpm_tty);
		pr_debug("%s BLUETOOTH:Calling to flow ON RTS.\n", __func__);
	}
	return 0;
}
...
static struct platform_driver bcm_bt_lpm_platform_driver = {
	...
	.suspend = bcm_bt_lpm_suspend,
	.resume = bcm_bt_lpm_resume,
	.driver = {
		   .name = "bcm-bt-lpm",
		   .owner = THIS_MODULE,
