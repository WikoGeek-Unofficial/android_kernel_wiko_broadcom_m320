...
static const struct of_device_id hscdtd008a_of_match[] = {
	{.compatible = "alps,hscdtd008a"},
	{},
};
MODULE_DEVICE_TABLE(of, hscdtd008a_of_match);
...
{
	struct hscd_i2c_platform_data *platform_data = NULL;
        int res = 0;
    	int ii = 0;
	struct device_node *np;
	const struct of_device_id *match = of_match_device(hscdtd008a_of_match,
				&client->dev);
...
static struct i2c_driver hscd_driver = {
	...
	.driver 	= {
		.owner	= THIS_MODULE,
		.name	= HSCD_DRIVER_NAME,
		.of_match_table = of_match_ptr(hscdtd008a_of_match),
                .pm = &hscd_pm_ops,
	},
};
