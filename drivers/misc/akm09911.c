...
#define AK09911_MEASUREMENT_TIME_US (msecs_to_jiffies(20))
...
#ifdef AKM_RUNTIME_PM
static struct regulator *ak09911_regltr_3v;
static struct regulator *ak09911_regltr_18v;
static int ak09911_power_on_off(bool flag);
static int ak09911_cs_power_on_off(bool flag);
#endif
...
static int ak09911_set_power_down(
	struct akm_compass_data *akm)
{
	uint8_t buffer[2];
	int err;
...
#else
	err = ak09911_set_power_down(akm);
#endif

	return err;
}

static int ak09911_set_mode(
	struct akm_compass_data *akm,
	uint8_t mode)
{
	int err;
...
static int ak09911_get_data_poll(
	struct akm_compass_data *akm,
	uint8_t *rbuf,
	int size)
{
...
static int ak09911_get_open_status(
	struct akm_compass_data *akm)
{
...
static int ak09911_get_close_status(
	struct akm_compass_data *akm)
{
	return wait_event_interruptible(
			akm->open_wq, (atomic_read(&akm->active) <= 0));
...
static int ak09911_open(struct inode *inode, struct file *file)
{
	file->private_data = s_akm;
	return nonseekable_open(inode, file);
}

static int ak09911_release(struct inode *inode, struct file *file)
{
	return 0;
}

static long
ak09911_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_RESET:
		...
		ret = ak09911_reset(akm, akm->gpio_rstn);
		if (ret < 0)
			return ret;
		break;
	case ECS_IOCTL_SET_MODE:
		dev_vdbg(&akm->i2c->dev, "IOCTL_SET_MODE called.");
		ret = ak09911_set_mode(akm, mode);
		if (ret < 0)
			return ret;
		break;
	case ECS_IOCTL_SET_YPR:
		dev_vdbg(&akm->i2c->dev, "IOCTL_SET_YPR called.");
		ak09911_set_ypr(akm, ypr_buf);
		break;
	case ECS_IOCTL_GET_DATA:
		dev_vdbg(&akm->i2c->dev, "IOCTL_GET_DATA called.");
		if (akm->irq_gpio)
			ret = ak09911_get_data(akm, dat_buf,
				AKM_SENSOR_DATA_SIZE);
		else
			ret = ak09911_get_data_poll(
	...
	case ECS_IOCTL_GET_OPEN_STATUS:
		...
		ret = ak09911_get_open_status(akm);
		if (ret < 0) {
			dev_err(&akm->i2c->dev,
	...
	case ECS_IOCTL_GET_CLOSE_STATUS:
		...
		ret = ak09911_get_close_status(akm);
		if (ret < 0) {
			dev_err(&akm->i2c->dev,
...
static const struct file_operations ak09911_fops = {
	.owner = THIS_MODULE,
	.open = ak09911_open,
	.release = ak09911_release,
	.unlocked_ioctl = ak09911_ioctl,
};
	...
	char const *buf, size_t count)
{
	...
	if (ak09911_set_mode(akm, (uint8_t)mode) < 0)
		return -EINVAL;
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	err = ak09911_set_mode(akm, AK09911_MODE_FUSE_ACCESS);
	if (err < 0)
		return err;
	...
	err = ak09911_set_mode(akm, AK09911_MODE_POWERDOWN);
	if (err < 0)
		return err;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void ak09911_early_suspend(struct early_suspend *handler)
{
	int err;
	pr_info("ak09911_early_suspend\n");
	...
	err = ak09911_set_mode(s_akm, AK09911_MODE_POWERDOWN);
	if (err < 0)
		pr_info("ak09911_set_mode returned %d", err);
	wake_up(&s_akm->open_wq);
#ifdef AKM_SUSPEND_PM
	ak09911_power_on_off(0);
	ak09911_cs_power_on_off(0);
#endif
}

static void ak09911_late_resume(struct early_suspend *handler)
{
	pr_info("ak09911_late_resume\n");

#ifdef AKM_SUSPEND_PM
	ak09911_cs_power_on_off(1);
	ak09911_power_on_off(1);
#endif
...
{
	int ret;
	if (!ak09911_regltr_3v) {
		pr_err("ak09911_regltr_3v is unavailable\n");
		return -1;
	}
	if ((flag == 1)) {
		pr_info("\n LDO on %s ", __func__);
		ret = regulator_enable(ak09911_regltr_3v);
		if (ret)
			pr_err("ak09911 Regulator-enable failed\n");
	} else if ((flag == 0)) {
		pr_info("\n LDO off %s ", __func__);
		ret = regulator_disable(ak09911_regltr_3v);
		if (ret)
			pr_err("Ak09911 3v Regulator disable failed\n");
	}
	return 0;
}
...
{
	int ret;
	if (!ak09911_regltr_18v) {
		pr_err("ak09911_regltr_18v is unavailable\n");
		return -1;
	}
	if ((flag == 1)) {
		pr_info("\n LDO on %s ", __func__);
		ret = regulator_enable(ak09911_regltr_18v);
		if (ret)
			pr_err("ak09911 1v8 Regulator enable failed\n");
	} else if ((flag == 0)) {
		pr_info("\n LDO off %s ", __func__);
		ret = regulator_disable(ak09911_regltr_18v);
		if (ret)
			pr_info("ak09911 1v8 Regulator disable failed\n");
	}
	return 0;
}
#endif
...
#ifdef AKM_RUNTIME_PM
	pr_info("%s: ak09911 setting up regulators\n", __func__);
	ak09911_regltr_3v = regulator_get(NULL, "sensor_3v");
	if (IS_ERR_OR_NULL(ak09911_regltr_3v)) {
		pr_err("%s: ak09911 Failed to get regulator\n", __func__);
		return -EBUSY;
	}
	regulator_set_voltage(ak09911_regltr_3v, 3000000, 3000000);
	err = regulator_enable(ak09911_regltr_3v);
	if (err)
		pr_err("ak09911 Regulator Enable Failed\n");
	ak09911_regltr_18v = regulator_get(NULL, "sensor_1v8");
	if (IS_ERR_OR_NULL(ak09911_regltr_18v)) {
		pr_err("%s: ak09911 18v Failed to get regulator\n",
		__func__);
		err = -EBUSY;
		goto err_pwr;
	}
	regulator_set_voltage(ak09911_regltr_18v, 1800000, 1800000);
	err = regulator_enable(ak09911_regltr_18v);
	if (err)
		pr_err("ak09911 1v8 Regulator enable failed\n");
#endif
	...
	if (pdata) {
		...
		if (client->dev.of_node) {
			...
			if (of_property_read_u32(np, "gpio-irq-pin", &val)) {
				pr_info("ak09911: irq is not set in dts\n");
				s_akm->irq = 0;
			} else {
				gpio_request(val, "compass_int");
				gpio_direction_input(val);
				s_akm->irq_gpio = val;
				s_akm->irq = gpio_to_irq(val);
			}
			if (of_property_read_u32(np, "gpio_rstn", &val)) {
				pr_info("ak09911: rst is not set in dts\n");
				s_akm->gpio_rstn = 0;
			} else {
				pr_info("ak09911: rst:%d\n", val);
				s_akm->gpio_rstn = val;
			}
			if (of_property_read_u32(np, "layout", &val)) {
				pr_info("ak09911: layout is not set in dts\n");
				s_akm->layout = 1;
			} else {
				pr_info("ak09911: layout:%d\n", val);
				s_akm->layout = val;
			}
		} else {
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	s_akm->akm_early_suspend.suspend = ak09911_early_suspend;
	s_akm->akm_early_suspend.resume = ak09911_late_resume;
...
exit0:
	...
	if (ak09911_regltr_18v) {
		ak09911_cs_power_on_off(0);
		regulator_put(ak09911_regltr_18v);
	}
err_pwr:
	if (ak09911_regltr_3v) {
		ak09911_power_on_off(0);
		regulator_put(ak09911_regltr_3v);
	}
#endif
...
static const struct of_device_id ak09911_of_match[] = {
...
static struct i2c_driver akm_compass_driver = {
	...
	.driver = {
		.name	= AKM_I2C_NAME,
		.of_match_table = ak09911_of_match,
	},
};
