...
#ifdef custom_t_BUG_FIX
		if(error)
			WLDEV_ERROR(("%s: wldev_ioctl WLC_SET_BAND error %d\n",	__FUNCTION__, error));

		//we check if the band set right

		for(retry = 3;retry > 0;retry ++)
		{

			if(0 == wldev_get_band(dev,&getband))
			{
				if(getband == band)
				{
					WLDEV_ERROR(("%s: wldev_ioctl wldev_get_band return is %d , same with wldev_set_band %d. retry done\n",
						__FUNCTION__, getband,band));
					break;
				}
				else
				{
					WLDEV_ERROR(("%s: wldev_ioctl wldev_get_band return is %d , not same with wldev_set_band %d. retry again\n",
						__FUNCTION__, getband,band));
			...
			{
				WLDEV_ERROR(("%s: wldev_ioctl wldev_get_band error %d\n", __FUNCTION__, error));
			}
		}		
#endif
