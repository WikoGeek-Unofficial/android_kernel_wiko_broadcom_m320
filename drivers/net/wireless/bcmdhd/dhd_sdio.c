...
static int concate_revision_bcm43340(dhd_bus_t *bus,
		char *fw_path, int fw_path_len, char *nv_path, int nv_path_len)
{
	uint chipver;
#if defined(SUPPORT_MULTIPLE_CHIPS)
	char chipver_tag[10] = "_43340";
#else
	char chipver_tag[4] = {0, };
#endif /* defined(SUPPORT_MULTIPLE_CHIPS) */

	DHD_TRACE(("%s: BCM43340 Multiple Revision Check\n", __FUNCTION__));
	if (bus->sih->chip != BCM43340_CHIP_ID) {
		DHD_ERROR(("%s:Chip is not BCM43340\n", __FUNCTION__));
		return -1;
	}
	chipver = bus->sih->chiprev;
	DHD_ERROR(("CHIP VER = [0x%x]\n", chipver));
	if (chipver == 0x1) {
		DHD_ERROR(("----- CHIP bcm43340_A0 -----\n"));
		strcat(chipver_tag, "_a0");
	} else if (chipver == 0x2) {
		DHD_ERROR(("----- CHIP bcm43340_B0 -----\n"));
		strcat(chipver_tag, "_b0");
	} else {
		DHD_ERROR(("----- CHIP bcm43340 unknown revision %d -----\n",
			chipver));
	}
...
/* get the base name (prefix) for firmware & nvram from the platform */
	...
	case BCM4330_CHIP_ID:
		res = concate_revision_bcm4330(bus, fw_path, fw_path_len,
			nv_path, nv_path_len);
		break;
	...
	case BCM43340_CHIP_ID:
		res = concate_revision_bcm43340(bus, fw_path, fw_path_len, nv_path, nv_path_len);
		break;
