...
static const struct v4l2_ctrl_config rcu_custom_led_cfg = {
	...
	.name   = "RCU Set LED",
	.min    = INT_MIN,
	.max    = SH_RCU_LED_ON | SH_RCU_LED_MODE_PRE,
	.step   = 1,
	.def    = RCU_CUSTOM_CMD_DEFAULT_VALUE,
};
...
static const struct v4l2_ctrl_config rcu_custom_scenemodes_cfg = {
	.ops    = &sh_mobile_rcu_ctrl_ops,
	.id     = V4L2_CID_CAMERA_SCENE_MODES,
	.type   = V4L2_CTRL_TYPE_INTEGER,
	.name   = "RCU Set Camera Scenemodes",
	.min    = INT_MIN,
	.max    = INT_MAX,
	.step   = 1,
	.def    = RCU_CUSTOM_CMD_DEFAULT_VALUE,
...
static const struct v4l2_ctrl_config rcu_custom_iso_cfg = {
	.ops    = &sh_mobile_rcu_ctrl_ops,
	.id     = V4L2_CID_CAMERA_ISO,
	.type   = V4L2_CTRL_TYPE_INTEGER,
	.name   = "RCU Set Camera Iso",
	.min    = INT_MIN,
	.max    = INT_MAX,
	.step   = 1,
	.def    = RCU_CUSTOM_CMD_DEFAULT_VALUE,
...
static const struct v4l2_ctrl_config rcu_custom_metering_exposure_cfg = {
	.ops    = &sh_mobile_rcu_ctrl_ops,
	.id     = V4L2_CID_CAMERA_METERING_EXPOSURE,
	.type   = V4L2_CTRL_TYPE_INTEGER,
	.name   = "RCU Set Camera Metering",
	.min    = INT_MIN,
	.max    = INT_MAX,
	.step   = 1,
	.def    = RCU_CUSTOM_CMD_DEFAULT_VALUE,
...
static const struct v4l2_ctrl_config rcu_custom_cam_record_cfg = {
	.ops    = &sh_mobile_rcu_ctrl_ops,
	.id     = V4L2_CID_CAM_RECORD,
	.type   = V4L2_CTRL_TYPE_INTEGER,
	.name   = "RCU Set Camera Record ",
	.min    = INT_MIN,
	.max    = INT_MAX,
	.step   = 1,
	.def    = RCU_CUSTOM_CMD_DEFAULT_VALUE,
...
static const struct v4l2_ctrl_config rcu_custom_cam_get_flash_cfg = {
	.ops    = &sh_mobile_rcu_ctrl_ops,
	.id     = V4L2_CID_CAMERA_GET_FLASH_ONOFF,
	.type   = V4L2_CTRL_TYPE_INTEGER,
	.name   = "RCU Get Flash",
	.min    = INT_MIN,
	.max    = INT_MAX,
...
static int sh_mobile_rcu_add_device(struct soc_camera_device *icd)
{
	...
	pcdev->rcu_early_suspend_clock_state =
		cpufreq_set_clocks_mode(CPUFREQ_MODE_ES_SUPPRESS, true);
...
static void sh_mobile_rcu_remove_device(struct soc_camera_device *icd)
{
	...
	if (0 == pcdev->rcu_early_suspend_clock_state)
		cpufreq_set_clocks_mode(CPUFREQ_MODE_ES_SUPPRESS, false);
...
static int sh_mobile_rcu_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	struct sh_mobile_rcu_snd_cmd snd_cmd = {0, 0, NULL, 0 , NULL};
	...
	if (ctrl->val != RCU_CUSTOM_CMD_DEFAULT_VALUE) {
		...
		case V4L2_CID_SET_MMAP_PAGE:
			...
			if (sizeof(mmap_page_info) /
				sizeof(mmap_page_info[0]) <= 2) {
				page_num = (mmap_page_info[0] + PAGE_SIZE - 1)
					/ PAGE_SIZE;
			} else {
				dev_err(icd->parent,
					"%s:array mmap_page_info is exceeds 2 element\n",
					__func__);
				return -EIO;
			}
		...
		unsigned int idx, struct soc_camera_format_xlate *xlate)
{
	...
	v4l2_ctrl_new_custom(&icd->ctrl_handler,
				&rcu_custom_scenemodes_cfg, NULL);
	...
	v4l2_ctrl_new_custom(&icd->ctrl_handler,
			&rcu_custom_metering_exposure_cfg, NULL);
	...
	v4l2_ctrl_new_custom(&icd->ctrl_handler,
				&rcu_custom_cam_get_flash_cfg, NULL);

	v4l2_ctrl_new_custom(&icd->ctrl_handler,
				&rcu_custom_cam_record_cfg, NULL);

	v4l2_ctrl_new_custom(&icd->ctrl_handler,
				&rcu_custom_iso_cfg, NULL);
...
static int sh_mobile_rcu_runtime_resume(struct device *dev)
{
	...
	if (NULL != pcdev->iclk) {
		ret = clk_enable(pcdev->iclk);
		if (0 != ret) {
			pr_err("%s :clk_enable(iclk) err(%d)",
				 __func__, ret);
			return ret;
		}
	} else {
	...
	if (NULL != pcdev->fclk) {
		ret = clk_enable(pcdev->fclk);
		if (0 != ret) {
			pr_err("%s :clk_enable(fclk) err(%d)",
				 __func__, ret);
			return ret;
		}
	} else {
	...
	if (NULL != pcdev->mclk) {
		ret = clk_enable(pcdev->mclk);
		 if (0 != ret) {
			pr_err("%s :clk_enable(mclk) err(%d)",
				 __func__, ret);
			return ret;
		}
	} else {
