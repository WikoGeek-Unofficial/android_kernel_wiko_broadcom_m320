...
{
	int i;
 //++  add for elan tp tpgrade report "Polling timed out"  by suwei 5.9.2014
#ifdef custom_t_KERNEL_I2C 
	for (i = 4000; i; i--) {
#else
	for (i = 1000; i; i--) {
#endif
//--  add for elan tp tpgrade report "Polling timed out"  
		u_int8_t val = iic_rd(pd, ICSR);
			      ...
			      int num)
{
	...
	(struct platform_device *) container_of(pd->dev, struct platform_device, dev);
	int irq;
	...
	for (i = 0; i < num; i++) {
		...
		if (unlikely(irqs_disabled())) {
			/* machine poweroff needs to run with interrupt
			 * disabled. this block does an i2c transfer in
			 * poll mode.
			 */
			unsigned int timeout = 0;
			do {
				irq =  iic_rd(pd, ICSR) & iic_rd(pd, ICIC) &
					(ICSR_AL | ICSR_TACK | ICSR_WAIT
					| ICSR_DTE);
				if (irq)
					sh_mobile_i2c_isr(-1, dev);

				k = (pd->sr & SW_DONE) != 0;
				if (k)
					break;
			       udelay(10);
			  } while (timeout++ <= pd->timeout * 100);
		} else
...
static int sh_mobile_i2c_probe(struct platform_device *dev)
{
	...
	if (bus_data_delay <= MAX_SDA_DELAY)
