...
static struct r8a66597_platdata *r8a66597_hcd_of_init(
...
static void irq_vbus_state(struct r8a66597 *r8a66597)
{
	u16 syssts;
	u16 vbussts;
	struct usb_phy *x = usb_get_phy(0);
	...
	switch (vbussts) {
		case VB_SESS_END:
			...
			"%s<<<<<<\n", __func__, __LINE__, \
				usb_otg_state_string(x->state), \
			usb_otg_state_string(OTG_STATE_A_IDLE));
			x->state = OTG_STATE_A_IDLE;
			hcd_log("%s\n", usb_otg_state_string(x->state));
			break;
		case VSESS_VLD:
			 pr_debug("\n>>> HSUSB:HCD: %s(%d): OTG_STATE == %s<<<<<<\n",
			 __func__, __LINE__, usb_otg_state_string(x->state));
		...
		case VA_VBUS_VLD:
			...
			"%s -> %s <<<<<<\n", __func__, __LINE__,\
			usb_otg_state_string(x->state),\
			usb_otg_state_string(OTG_STATE_A_WAIT_BCON));
	...
	if (mask1) {
		if (mask1 & ATTCH) {
			...
			if ((x->state == OTG_STATE_B_WAIT_ACON ||
				...
				"\x1b[0m = %s -> %s <<<<<<\n", __func__,
				 __LINE__, usb_otg_state_string(x->state),
				usb_otg_state_string(OTG_STATE_A_HOST));
...
static void r8a66597_srp_work(struct work_struct *work)
{
	...
	__func__, __LINE__, usb_otg_state_string(x->state), \
	usb_otg_state_string(OTG_STATE_A_WAIT_VRISE));
...
static void r8a66597_hnp_work(struct work_struct *work)
{
	...
	if (otg->state == OTG_STATE_B_WAIT_ACON) {
		pr_debug("\n>>> HSUSB:HCD: %s(%d): OTG_STATE\x1b [0m = %s -> %s <<<<<<\n",
			 __func__, __LINE__, usb_otg_state_string(otg->state),
			usb_otg_state_string(OTG_STATE_B_HOST));
...
static int r8a66597_bus_suspend(struct usb_hcd *hcd)
{
	...
	for (port = 0; port < r8a66597->max_root_hub; port++) {
		...
		if (otg->state == OTG_STATE_A_HOST) {
			...
			"%s -> %s <<<<<<\n", __func__, __LINE__, \
			usb_otg_state_string(otg->state), \
			usb_otg_state_string(OTG_STATE_A_SUSPEND));
...
static int r8a66597_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		r8a66597->pdata = r8a66597_hcd_of_init(pdev);
		if (IS_ERR(r8a66597->pdata)) {
			dev_err(&pdev->dev, "platform data not available\n");
			return PTR_ERR(r8a66597->pdata);
		}
	} else {
...
r8a66597_hcd_of_init(struct platform_device *pdev) {
...
static const struct of_device_id r8a66597_hcd_of_match[] = {
	{ .compatible = "renesas,r8a66597-hcd" },
	{}
};
MODULE_DEVICE_TABLE(of, r8a66597_hcd_of_match);
#endif
...
static struct platform_driver r8a66597_driver = {
	...
	.driver		= {
		.name = (char *) hcd_name,
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(r8a66597_hcd_of_match),
