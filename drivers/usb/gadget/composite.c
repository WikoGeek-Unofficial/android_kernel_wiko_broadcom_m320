...
void usb_gstrings_detach(struct usb_composite_dev *cdev)
{
	struct usb_gadget_string_container *uc, *tmp;
	/* To prevent overwriting of iSerial id incrementing id by one */
	cdev->next_string_id = USB_GADGET_FIRST_AVAIL_IDX+1;
	list_for_each_entry_safe(uc, tmp, &cdev->gstrings, list) {
		list_del(&uc->list);
		kfree(uc);
	}
}
EXPORT_SYMBOL_GPL(usb_gstrings_detach);
		...
		enum usb_device_speed speed, void *buf, u8 type)
{
	...
	list_for_each_entry(f, &config->functions, list) {
		...
		if (change_conf(f, next, len, config, speed) < 0) {
			NPRINTK("failed to change configuration\n");
			return -EINVAL;
		}
#endif
		if (status < 0)
