...
#define MANUFACTURER_STRING "KITE"
#define PRODUCT_STRING "KITE"
#else
#define MANUFACTURER_STRING "Android"
#define PRODUCT_STRING "Android"
#endif
...
static int mass_storage_function_reinit(struct android_usb_function *f,
					struct usb_composite_dev *cdev,
					int nluns)
{
	struct mass_storage_function_config *config;
...
static ssize_t mass_storage_luns_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifdef custom_t_BUG_FIX
	...
	struct android_usb_function *f = dev_get_drvdata(dev);
	struct mass_storage_function_config *config =
		(struct mass_storage_function_config *)f->config;

	return sprintf(buf, "%d\n", config->fsg.nluns);
#endif	
}

static ssize_t mass_storage_luns_store(struct device *dev,
		...
		const char *buf, size_t count)
{
	...
	if (adev->enabled) {
		rc = -EBUSY;
	} else if (nluns <= 0 || nluns > FSG_MAX_LUNS) {
		pr_err("Only support 1 - %d luns. Input %d\n",
				FSG_MAX_LUNS, nluns);
		rc = -EINVAL;
	} else {
		mass_storage_function_reinit(f, cdev, nluns);
		rc = count;
	}
...
static DEVICE_ATTR(luns, S_IRUGO | S_IWUSR,
		mass_storage_luns_show,
		mass_storage_luns_store);
...
char androidbootmode[20];
EXPORT_SYMBOL(androidbootmode);

static int __init androidboot_setup(char *str)
{
	memset(androidbootmode,0,sizeof(androidbootmode));
	strncpy(androidbootmode,str,sizeof(androidbootmode));
	printk("custom_t:androidbootmode return %s\n",androidbootmode);

	return 0;
}

__setup("androidboot.mode=", androidboot_setup);
...
#ifdef custom_t_CUSTOM
		printk("custom_t:usb_string_id return %d\n",id);
	
		if(0 != strncmp(androidbootmode,"normal_mode",strlen(androidbootmode)))
		{
			//if not normal mode , we need use different serial number
