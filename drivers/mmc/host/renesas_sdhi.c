...
static u8 sdcard1_detect_state;
...
static void sdhi_write16(struct renesas_sdhi_host *host, u32 offset, u16 val)
{
	...
	case SDHI_SDIO_MODE:
	case SDHI_EXT_ACC:
		while (((sdhi_read16(host, SDHI_INFO2) << 16)
				& SDHI_INFO_DIVEN) == 0) {
			if (timeout++ > 43)
				break;
			udelay(1);
...
static int sdhi_restore_register(struct renesas_sdhi_host *host)
{
	...
	sdhi_write16(host, SDHI_INFO2,
		sdhi_read16(host, SDHI_INFO2) | (1 << 13));
...
static void renesas_sdhi_detect_work(struct work_struct *work)
{
	...
	pr_info("%s: %s: host->connect %d, get_cd %d\n",
...
{
	return snprintf(buf, (sizeof(sdcard1_detect_state) + 1), "%d\n",
...
static int renesas_sdhi_probe(struct platform_device *pdev)
{
	...
	if (pdata->card_stat_sysfs) {
		...
		if (sysfs_ret)
			pr_err("SYSFS initialization for SDHI:sdcard1 detection failed\n");
	}
