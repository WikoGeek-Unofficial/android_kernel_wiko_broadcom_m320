	...
	size_t count, loff_t *offset)
{
	...
	if (*buf1 == '+') {
		...
		if (kstrtoul(buf1+1, 10, &g_email_block_enabled) < 0)
			return count;
	} else {
		if (kstrtoul(buf1, 10, &g_email_netlink_pid) < 0)
			return count;
	}
