...
static struct resource *mfi_resource;
static struct resource *sh_firm_resource;
...
static int __init rt_sub_init(void)
{
	...
	mfi_resource = get_mem_resource("mfi");
	sh_firm_resource = get_mem_resource("sh_firm");
...
)
{
	...
	rtds_memory_drv_ioctl_cache		ioctl_cache = {0, 0};
	...
	if (ioctl_cache.app_addr > UINT_MAX
		|| ioctl_cache.mem_size > UINT_MAX) {
		ret_code = -EINVAL;
		return ret_code;
	}
	/* Flush L2 cache. */
	rtds_memory_drv_flush_cache((unsigned int)ioctl_cache.app_addr,
					ioctl_cache.mem_size);
...
)
{
	...
	/* Clear the L2 cache. */
	rtds_memory_drv_inv_cache((unsigned int)ioctl_cache.app_addr,
					ioctl_cache.mem_size);
	...
	if (current->flags & PF_EXITING) {
		...
		list_for_each_entry(mem_table, &g_rtds_memory_list_shared_mem,
			...
			if ((mem_table->task_info->tgid == current->tgid) &&
				...
				MSG_ERROR("[RTDSK]IN |[%s]:flags[0x%08X],\
				tgid[%d], pid[%d] \n", __func__,
				mem_table->task_info->flags,
				mem_table->task_info->tgid,
				mem_table->task_info->pid);

				MSG_ERROR("[RTDSK]IN |[%s]:state[0x%08x],\
				rt_wb_addr[0x%08x],rt_nc_addr[0x%08x] \n",
				__func__, (u32)mem_table->task_info->state,
				(u32)mem_table->rt_wb_addr,
				(u32)mem_table->rt_nc_addr);
...
int rtds_memory_change_rtpmb_to_phy_address(
	...
	if ((mfi_resource->start+mfi_offset <= rtpmb_addr)
		&& (rtpmb_addr <= mfi_resource->end+mfi_offset)) {
		*phy_addr = rtpmb_addr - mfi_offset;
	} else if ((sh_firm_resource->start+shnc_offset <= rtpmb_addr)
		&& (rtpmb_addr <= sh_firm_resource->end+shnc_offset)) {
		*phy_addr = rtpmb_addr - shnc_offset;
	} else if ((sh_firm_resource->start+sh_offset  <= rtpmb_addr)
		&& (rtpmb_addr <= sh_firm_resource->end+sh_offset)) {
...
int rtds_memory_change_phy_to_rtpmb_address(
	...
	if ((mfi_resource->start <= phy_addr)
		&& (phy_addr <= mfi_resource->end)) {
		*rtpmb_addr = phy_addr + mfi_offset;
	} else if ((sh_firm_resource->start <= phy_addr)
		&& (phy_addr <= sh_firm_resource->end)) {
...
int rtds_memory_change_rtpmb_cache_address(
	...
	if ((sh_firm_resource->start+shnc_offset <= rtpmb_addr)
		&& (rtpmb_addr <= sh_firm_resource->end+shnc_offset)) {
...
)
{
	...
	list_for_each_entry_safe(mem_table, temp_p,
		...
		if ((mem_table->task_info->flags & PF_EXITPIDONE) &&
			...
			MSG_ERROR("[RTDSK]IN |[%s]:flags[0x%08X],tgid[%d],\
			pid[%d] \n", __func__, mem_table->task_info->flags,
			mem_table->task_info->tgid, mem_table->task_info->pid);
...
#else
	if ((mem_table->event == RTDS_MEM_MAP_PNC_EVENT)
		|| (mem_table->event == RTDS_MEM_UNMAP_PNC_EVENT)
#ifdef RTDS_SUPPORT_DRM
...
)
{
	...
	*phys_addr  = rt_cma_drv_alloc(id, mem_size, &cma_list->cpu_addr);
	if (0 == *phys_addr) {
		MSG_ERROR("[RTDSK]ERR| rt_cma_drv_alloc failed\n");
		kfree(cma_list);
		ret = SMAP_MEMORY;
		goto out;
	}
	...
	if (RTDS_MEM_ATTR_MEDIA == cma_list->mem_attr) {
		...
		if (SEC_HAL_CMN_RES_OK != sec_ret) {
			...
			ret = rt_cma_drv_free(cma_list->id,
						cma_list->mem_size,
						cma_list->phy_addr,
						cma_list->cpu_addr);
			kfree(cma_list);
...
)
{
	...
	ret = rt_cma_drv_free(id, mem_size, phys_addr, cpu_addr);
	if (SMAP_OK != ret) {
		MSG_ERROR("[RTDSK]ERR| rt_cma_drv_free failed\n");
		ret = SMAP_NG;
	}
	kfree(cma_list);
