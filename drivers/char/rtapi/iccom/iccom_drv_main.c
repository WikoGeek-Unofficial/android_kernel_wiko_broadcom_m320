...
#define ICCOM_MFIS_MAX_SIZE	2048
		...
		case ICCOM_CMD_SEND:
			{
				...
				if (send_size > ICCOM_MFIS_MAX_SIZE) {
					ret = -EINVAL;
					return ret;
				}
					...
					if (copy_from_user(send_data,
						(void __user *)
						ioctl_param.send.send_data,
						(ulong)send_size)) {
		...
		case ICCOM_CMD_RECV:
			{
				...
				if (copy_from_user(&ioctl_param.recv,
					(void __user *) arg,
					(ulong)sizeof(ioctl_param.recv))) {
					MSG_ERROR("[ICCOMK]ERR| ioctl copy error.\n");
					ret = SMAP_NG;
					break;
				}
		...
		case ICCOM_CMD_RECV_ASYNC:
			{
				...
				if (SMAP_OK == ret) {
					...
					if (copy_to_user((void __user *)arg,
					&ioctl_param.recv_async,
					(ulong)sizeof(ioctl_param.recv_async))
					) {
						MSG_ERROR("[ICCOMK]ERR| output data copy.\n");
						ret = SMAP_NG;
					}
		...
		case ICCOM_CMD_GET_RECV_DATA:
			{
				...
				if (copy_from_user(&ioctl_param.get_data,
					(void __user *) arg,
					(ulong)sizeof(ioctl_param.get_data))) {
					MSG_ERROR("[ICCOMK]ERR| ioctl copy error.\n");
					ret = SMAP_NG;
					break;
				}
				if ((NULL != ioctl_param.get_data.recv_data) && (NULL != ioctl_param.get_data.user_recv_data)) {
					if (ioctl_param.get_data.recv_size >
								UINT_MAX) {
						ret = -EINVAL;
						return ret;
					}
					...
					if (copy_to_user(
					(void __user *)
					ioctl_param.get_data.user_recv_data,
					 ioctl_param.get_data.recv_data,
					(ulong)ioctl_param.get_data.recv_size)
					) {
						MSG_ERROR("[ICCOMK]ERR| output data copy.\n");
						ret = SMAP_NG;
					}
		...
		case ICCOM_CMD_SET_FATAL:
			{
				...
				if (copy_from_user(&ioctl_param.set_fatal,
					(void __user *) arg,
					(ulong)sizeof(ioctl_param.set_fatal))
				) {
					MSG_ERROR("[ICCOMK]ERR| ioctl copy error.\n");
					ret = SMAP_NG;
					break;
		...
		case ICCOM_CMD_SET_THREAD:
			{
				...
				if (copy_from_user(&ioctl_param.set_thread,
				(void __user *) arg,
				(ulong)sizeof(ioctl_param.set_thread))) {
					MSG_ERROR("[ICCOMK]ERR| ioctl copy error.\n");
					ret = SMAP_NG;
					break;
				}
