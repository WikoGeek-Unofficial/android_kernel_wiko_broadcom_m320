...
static unsigned long g_mms_netlink_pid;
static unsigned long g_mms_block_enabled;
	...
	size_t count, loff_t *offset)
{
	...
	if (*buf1 == '+') {
		if (kstrtoul(buf1+1, 10, &uid_in) < 0)
			return count;
		...
		unsigned long blocked_uid = 0;
		struct netlink_event mmsevent;
		if (kstrtoul(buf1+1, 10, &blocked_uid) < 0)
			return count;
		mmsevent.type = 0;
		mmsevent.uid = blocked_uid;
		send_netlinkevent_to_security_center(&mmsevent);
	} else if (*buf1 == '#') {
		if (kstrtoul(buf1+1, 10, &g_mms_block_enabled) < 0)
			return count;
	} else {
		if (kstrtoul(buf1, 10, &g_mms_netlink_pid) < 0)
			return count;
	}
	return count;
}
	...
	int (*okfn)(struct sk_buff *))
{
	...
	if (strnstr(tcp_payload,
		...
		if (strnstr(tcp_payload, "Content-Length: 20", 300)) {
			return NF_ACCEPT;
		}
