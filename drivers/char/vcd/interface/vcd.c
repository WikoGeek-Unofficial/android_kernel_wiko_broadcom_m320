 ...
 * @retval      none.
 */
static void vcd_update_buffer(unsigned char *pstr, unsigned int size)
{
	unsigned int counter;

	if ((pstr == NULL) || (size < VCD_PROC_BUF_SIZE)) {
		vcd_pr_err("Invalid Args. pstr=0x%x size=%d\n",
				...
				size_t len, loff_t *ppos)
{
	...
	if (len > VCD_PROC_BUF_SIZE)
		len = VCD_PROC_BUF_SIZE;

	proc_buf = kmalloc(len, GFP_KERNEL);
	if (proc_buf == NULL || buffer == NULL || len <= 0 ||
		len >= VCD_PROC_BUF_SIZE) {
		vcd_pr_err(" Invalid buffer OR size over len[%d].\n", len);
		goto rtn;
	}

	memset(proc_buf, 0, VCD_PROC_BUF_SIZE);

	/* copy from user */
	if (copy_from_user(proc_buf, buffer, len)) {
		vcd_pr_err("copy_from_user failed.\n");
		goto rtn;
	}
	vcd_update_buffer(proc_buf, VCD_PROC_BUF_SIZE);
	/* get exec proc */
	ret = kstrtouint((const char *)proc_buf, 0, &exec_proc);
	if (0 != ret) {
		vcd_pr_err("kstrtouint error=%d proc_buf[%s]\n", ret, proc_buf);
		goto rtn;
	}
...
#ifdef __PRINT_VCD__
	len = snprintf(page, sizeof(page), "0x%x\n", g_vcd_log_level);
#endif /* __PRINT_VCD__ */
				...
				size_t len, loff_t *ppos)
{
	...
	proc_buf = kmalloc(VCD_PROC_BUF_SIZE, GFP_KERNEL);
	...
	/* copy from user */
	if (copy_from_user((void *)proc_buf, buffer, len)) {
		vcd_pr_err("copy_from_user failed.\n");
		goto rtn;
	}
	vcd_update_buffer(proc_buf, VCD_PROC_BUF_SIZE);
	/* get log level */
	ret = kstrtouint((const char *)proc_buf, 0, &temp_log_level);
	if (0 != ret) {
		vcd_pr_err("kstrtouint error=%d proc_buf[%s]\n", ret, proc_buf);
		goto rtn;
	}
				...
				size_t len, loff_t *ppos)
{
	...
	ret = kstrtouint((const char *)proc_buf, 0, &write_func);
	if (0 != ret) {
		vcd_pr_err("kstrtouint error=%d proc_buf[%s]\n", ret, proc_buf);
		goto rtn;
	}
