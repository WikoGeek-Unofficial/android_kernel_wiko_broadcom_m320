...
/**
 * @brief       check pcm_proc_dual_mic.bin function.
 *
 * @param       none.
 *
 * @retval      1 if pcm_proc_dual_mic.bin is present else 0.
 */

int dual_mic_is_present(void)
{
	pr_info("%s() : dual_mic = %d\n", __func__, dual_mic);
	return dual_mic;
}
...
int vcd_spuv_set_binary_preprocessing(char *file_path)
{
	...
	/*pcm_proc_dual_mic.bin*/
	comp_result = strstr(file_path,
		(char *)VCD_SPUV_FUNC_PCM_PROC_DUAL_MIC_FILE_NAME);
	if (NULL != comp_result) {
		vcd_pr_spuv_info("set binary : [%s].\n",
		VCD_SPUV_FUNC_PCM_PROC_DUAL_MIC_FILE_NAME);
		dual_mic = 1;

		/* set information */
		addr = vcd_spuv_func_get_pcm_static_dual_mic_buffer();
		g_vcd_spuv_binary_info.binary_kind = VCD_BINARY_DUAL_MIC_PCM;
		g_vcd_spuv_binary_info.top_logical_address = addr;
		g_vcd_spuv_binary_info.top_physical_address = __pa(addr);
		g_vcd_spuv_binary_info.write_address = addr;
		g_vcd_spuv_binary_info.max_size = VCD_SPUV_FUNC_FW_BUFFER_SIZE;
