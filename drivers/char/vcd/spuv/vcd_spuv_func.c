...
static void *g_spuv_func_pcm_dual_mic_static_buffer;
...
int vcd_spuv_func_set_fw(void)
{
	...
	if (dual_mic_is_present()) {
		/*read pcm_proc_dual_mic.bin*/
		ret = vcd_spuv_func_read_binary(
					g_spuv_func_pcm_dual_mic_static_buffer,
					VCD_SPUV_FUNC_PCM_DUAL_MIC_SD_PATH);
		if (0 != ret) {
			vcd_pr_err("read pcm_proc.bin error. ret[%d].\n", ret);
			ret = VCD_ERR_SYSTEM;
			goto rtn;
		...
		/* pcm_proc_dual_mic.bin analyze */
		ret = vcd_spuv_func_fw_analyze(
			&read_fw_info,
			g_spuv_func_pcm_dual_mic_static_buffer);
		if (0 != ret) {
			vcd_pr_err(
				"pcm_proc.bin analyze error. ret[%d].\n", ret);
				ret = VCD_ERR_SYSTEM;
				goto rtn;
		}
	}
...
int vcd_spuv_func_set_fw(void)
{
	...
	if (dual_mic_is_present()) {
		/* pcm_proc_dual_mic.bin analyze */
		ret = vcd_spuv_func_fw_analyze(&read_fw_info,
			(char *)g_spuv_func_pcm_dual_mic_static_buffer);
		if (0 != ret) {
			vcd_pr_err(
				"pcm_proc.bin analyze error. ret[%d].\n", ret);
				ret = VCD_ERR_SYSTEM;
...
void *vcd_spuv_func_get_pcm_static_dual_mic_buffer(void)
{
	vcd_pr_start_spuv_function();

	vcd_pr_end_spuv_function();
	return g_spuv_func_pcm_dual_mic_static_buffer;
}
...
int vcd_spuv_func_ioremap(void)
{
	...
	g_spuv_func_sdram_static_area_top_phy =
			get_mem_resource("vocoder")->start;
	...
	g_spuv_func_sdram_diamond_area_top =
		(void __force *)ioremap_nocache(
			get_mem_resource("diamond")->start,
...
int vcd_spuv_func_get_fw_buffer(void)
{
	...
	/* get buffer */
	alloc_adr =
		vmalloc(VCD_SPUV_FUNC_FW_BUFFER_SIZE*3);
	if (NULL == alloc_adr) {
		vcd_pr_err("vmalloc error.\n");
		g_spuv_func_spuv_static_buffer = NULL;
		g_spuv_func_pcm_static_buffer = NULL;
		g_spuv_func_pcm_dual_mic_static_buffer = NULL;
		ret = -EBUSY;
		goto rtn;
	}
	memset(alloc_adr,
		0, VCD_SPUV_FUNC_FW_BUFFER_SIZE*3);
	...
	g_spuv_func_pcm_dual_mic_static_buffer = (void *)(
		(unsigned int)alloc_adr +
		2 * VCD_SPUV_FUNC_FW_BUFFER_SIZE);
...
void vcd_spuv_func_dump_fw_static_buffer_memory(void)
{
	...
	if (dual_mic_is_present()) {
		/* open file */
		file = filp_open("/mnt/sdcard/pcm_static_dual_mic_buffer.bin",
				 (O_WRONLY | O_LARGEFILE | O_CREAT), 0666);
		if (NULL == file) {
			vcd_pr_err("file open error.\n");
			goto rtn;
