...
int vcd_ctrl_set_binary_preprocessing(char *file_path)
{
	...
	else if (VCD_BINARY_DUAL_MIC_PCM == binary_kind)
		command = VCD_CTRL_FUNC_SET_BINARY_DUAL_MIC_PCM;
	else
...
int vcd_ctrl_set_binary_postprocessing(void)
{
	...
	if (VCD_BINARY_SPUV == binary_kind)
		...
		vcd_ctrl_func_set_active_feature(
			VCD_CTRL_FUNC_FEATURE_SET_BINARY_DUAL_MIC_PCM);
	else
		/* update active status */
		vcd_ctrl_func_set_active_feature(
...
/**
 * @brief	Get Vocoder current state.
 *
 * @param[in]	None.
 *
 * @retval	 Vocoder current state

 */
int vcd_ctrl_get_state(void)
{
	int ret;

	ret = vcd_ctrl_func_get_active_feature();
	pr_info("%s state = %d\n", __func__, ret);
