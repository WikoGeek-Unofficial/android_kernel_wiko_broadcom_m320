...
#define X_MAX_VALUE 540
#define Y_MAX_VALUE 960
#define VIRTUAL_KEY_HEIGHT  200
					...
					u8 finger_state, u8 n_finger)
{
	...
	input_report_abs(sensor->input, ABS_MT_POSITION_X, x*X_MAX_VALUE/(sensor->max_x));
	input_report_abs(sensor->input, ABS_MT_POSITION_Y, y*Y_MAX_VALUE/(sensor->max_y-VIRTUAL_KEY_HEIGHT));
...
static void f11_set_abs_params(struct rmi_function_container *fc, int index)
{
	...
	input_set_abs_params(input, ABS_MT_POSITION_X,
			x_min, X_MAX_VALUE, 0, 0);
	input_set_abs_params(input, ABS_MT_POSITION_Y,
			y_min, Y_MAX_VALUE, 0, 0);
	printk("RMI--x_min=%d,x_max=%d, y_min=%d, y_max=%d \n", x_min, x_max, y_min, y_max);
#ifdef	CONFIG_RMI4_F11_PEN
	if (sensor->sens_query.query9.has_pen)
...
int __init rmi_f11_module_init(void)
{
	int error;
...
void __exit rmi_f11_module_exit(void)
{
