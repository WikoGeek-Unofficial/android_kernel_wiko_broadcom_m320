...
#define TSP_RESET_PIN  21
static int tsp_reset_set(int level)
{
	int retval=0;

		retval = gpio_request(TSP_RESET_PIN, "rmi4_reset");
		if (retval) {
		...
		if (level)
			retval = gpio_direction_output(TSP_RESET_PIN, 1);
		else
			retval = gpio_direction_output(TSP_RESET_PIN, 0);
		
		if (retval) {
			pr_err("%s: Failed to setup reset gpio21. Code: %d.",
			       __func__, retval);
			gpio_free(TSP_RESET_PIN);
		}
...
static int tsp_power_control(int enable)
{
	...
	if (touch_regulator == NULL) {
		...
		if(IS_ERR(touch_regulator)){
			pr_err("RMI--failed to get regulator for Touch Panel\n");
	...
	if (enable) {
		...
		if (ret) {
			pr_err("RMI--failed to set voltage for vtsp_3v, err %d\n", ret);
			goto put;
		}
		ret = regulator_enable(touch_regulator);
		if (ret) {
			pr_err("RMI--failed to enable vtsp_3v, err %d\n", ret);
			goto put;
		}
	}else {
		ret = regulator_disable(touch_regulator);
		if (ret) {
			pr_err("RMI--failed to disable vtsp_3v, err %d\n", ret);
			goto put;
		}
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (pdata->attn_gpio > 0) {
		...
		if (error < 0) {
			dev_err(&client->dev,
				"request_threaded_irq failed %d--error=%d\n",
				pdata->attn_gpio, error );
			goto err_unregister;
		}
	}
...
extern int __init rmi_f11_module_init(void);
extern void __exit rmi_f11_module_exit(void);
extern int __init rmi_driver_init(void);
extern void __exit rmi_driver_exit(void);
extern int __init rmidev_init(void);
extern void __exit rmidev_exit(void);
extern int __init rmi_bus_init(void);
extern void __exit rmi_bus_exit(void);
extern int __init rmi_f01_module_init(void);
extern void __exit rmi_f01_module_exit(void);
