...
static u16 gv_hw_id;
static u16 gv_fw_ver;
static u16 gv_minor_fw_ver;
...
struct device *sec_touchkey_zintix;
...
retry_init:
	...
	gv_hw_id=touch_dev->cap_info.hw_id;
	...
	gv_minor_fw_ver=minor_firmware_version;
...
static void zinitix_late_resume(struct early_suspend *h)
{
	...
	pinctrl_pm_select_default_state(&touch_dev->client->dev);
	...
	zinitix_printk("HW_ID:%d, fw:%x, minor_fw:%x\n",gv_hw_id, gv_fw_ver, gv_minor_fw_ver);
...
#endif
	zinitix_printk("early suspend--\n");
	pinctrl_pm_select_sleep_state(&touch_dev->client->dev);
	up(&touch_dev->work_proceedure_lock);
	return;
}
...
static void run_reference_read(void *device_data) //SDND
{
	struct zinitix_touch_dev *info = (struct zinitix_touch_dev *)device_data;
	char buff[16] = {0};
...
static void run_reference_read_DND(void *device_data) //DND
{
	struct zinitix_touch_dev *info = (struct zinitix_touch_dev *)device_data;
	char buff[16] = {0};
	unsigned int min, max;
...
static const struct of_device_id zinitix_touch_of_match[] = {
	{.compatible = "zinitix,zinitix-touch",},
	{},
};

MODULE_DEVICE_TABLE(of, zinitix_touch_of_match);
		...
		const struct i2c_device_id *i2c_id)
{
	...
	touch_dev = devm_kzalloc(&client->dev,
		sizeof(struct zinitix_touch_dev), GFP_KERNEL);
	if (!touch_dev) {
	...
	/* initialize zinitix touch ic */
		...
		of_get_gpio(touch_dev->client->dev.of_node, 0);
	ret = gpio_request_one(touch_dev->int_gpio_num,
	GPIOF_IN, "Touch panel IRQ");
	if (ret) {
		zinitix_printk("unable to request IRQ gpio.\n");
	...
	sec_touchkey_zintix = device_create(sec_class, NULL, 0, touch_dev,
		"sec_touchkey");

	if (device_create_file(sec_touchkey_zintix,
...
static struct i2c_driver zinitix_touch_driver = {
	...
	.driver		= {
		...
		.of_match_table = zinitix_touch_of_match,
#ifdef ZCONFIG_PM_SLEEP
