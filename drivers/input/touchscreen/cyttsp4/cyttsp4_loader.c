...
#define CYTTSP4_AUTO_LOAD_FOR_CORRUPTED_CONFIG 1
		...
		struct cyttsp4_dev_id *dev_id)
{
	...
	if (status == ERROR_SUCCESS) {
		...
		dev_id->rev_id =
			status_buf[8];
		dev_id->silicon_id = get_unaligned_le16(&status_buf[6]);
		retval = 0;
	} else
		retval = -EIO;
	dev_vdbg(dev,
		 "%s: status=%d "
		 "bl_ver=%08X rev_id=%02X silicon_id=%04X\n",
		 __func__, status,
...
#define IMAGE_REC_SIZE ( (size_t)( CY_DATA_ROW_SIZE_TMA400 + (sizeof(struct cyttsp4_hex_image) - CY_DATA_ROW_SIZE) ) )

static int _cyttsp4_load_app(struct cyttsp4_device *ttsp, 
		struct cyttsp4_touch_firmware *platform_fws,
 		const u8 *bin_fw_img, int bin_fw_size)
{
	struct device *dev = &ttsp->dev;
#if 0//#ifdef DEBUG
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
	...
	if (!platform_fws && !bin_fw_img)
	{
		dev_err(dev,
			"%s: no platform fw, no bin fw\n", __func__);
		retval = -EINVAL;
	...
	dev_info(dev,
		"%s: dev: silicon id=0x%04X rev%s rev=0x%02X bl=0x%08X\n", 
		__func__, 
		dev_id->silicon_id, 
		dev_id->silicon_id < 0x0250 ? "B" : "D",
		dev_id->rev_id, dev_id->bl_ver);

	if (platform_fws)
	{
		if (dev_id->silicon_id < 0x0250) {
			fw_img = (const u8*)(platform_fws[CYTTSP4_SILICON_REV_B].img);
			fw_size = platform_fws[CYTTSP4_SILICON_REV_B].size;
			dev_info(dev, "%s: platform f/w for silicon B\n", __func__);
			dev_info(dev, "%s: fw ver = 0x%04x\n", __func__,
				platform_fws[CYTTSP4_SILICON_REV_B].fw_version);
		} else {
			fw_img = (const u8*)(platform_fws[CYTTSP4_SILICON_REV_D].img);
			fw_size = platform_fws[CYTTSP4_SILICON_REV_D].size;
			dev_info(dev, "%s: platform f/w for silicon D\n", __func__);
		}
	}
	else
	{
...
static void cyttsp4_fw_calibrate(struct cyttsp4_device *ttsp)
{
	...
	dev_info(dev, "%s: Calibration success\n", __func__);
...
//		rc = -EINVAL;
	...
	dev_info(dev, "%s: Baseline init success\n", __func__);
...
#define FW_DOWNLOAD_RETRY 3
	...
	while (retry)
	{
	rc = _cyttsp4_load_app(ttsp, platform_fws, bin_fw_img, bin_fw_size);
	if (rc == 0)
		break;
	...
	if (rc < 0) {
		...
		rc = cyttsp4_subscribe_attention(ttsp, CY_ATTEN_STARTUP,
				cyttsp4_fw_startup_attention, 0);
		if (rc < 0)
			dev_err(dev, "%s: Failed adding callback for startup\n",
					__func__);
	}
...
static void _cyttsp4_firmware_cont(const struct firmware *fw, void *context)
{
	...
	cyttsp4_upgrade_firmware(ttsp, 
		0, 
		&(fw->data[header_size + 1]),
		fw_size);
...
static int cyttsp4_fw_upgrade_platform(struct cyttsp4_device *ttsp, bool forced)
{
	struct device *dev = &ttsp->dev;
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
	...
	fws = cyttsp4_request_firmware(ttsp);
	if (fws == NULL) {
	...
	if (fws[CYTTSP4_SILICON_REV_D].img == NULL || fws[CYTTSP4_SILICON_REV_D].size == 0 ||
	    fws[CYTTSP4_SILICON_REV_B].img == NULL || fws[CYTTSP4_SILICON_REV_B].size == 0) {
		dev_err(dev, "%s: Platform firmware incorrect\n", __func__);
		rc = -EINVAL;
		goto exit;
	}

	if (fws[CYTTSP4_SILICON_REV_D].size % IMAGE_REC_SIZE != 0 ||
	    fws[CYTTSP4_SILICON_REV_B].size % IMAGE_REC_SIZE != 0 ) {
		dev_err(dev,
			"%s: Firmware image is misaligned\n", __func__);
		rc = -EINVAL;
		goto exit;
	...
	fw = &fws[CYTTSP4_SILICON_REV_D]; // select revD f/w by default.
	...
	siliconID = get_unaligned_be16(&data->si->si_ptrs.cydata->jtag_si_id3);
	hw_ver_ic = data->si->si_ptrs.samsung_data->hw_version;
	fw_ver_ic = get_unaligned_be16(&data->si->si_ptrs.samsung_data->fw_versionh);
	config_ver_ic = data->si->si_ptrs.samsung_data->config_version;
	dev_info(dev, "%s: silicon ID : 0x%04x, rev%s\n", __func__,
			siliconID, siliconID < 0x0250 ? "B" : "D");
	
	if (siliconID < 0x0250)
		fw = &fws[CYTTSP4_SILICON_REV_B];

	dev_info(dev, "%s: ic hw vers:0x%02X img hw vers:0x%02X\n", __func__,
			hw_ver_ic, fw->hw_version);
	dev_info(dev, "%s: ic fw vers:0x%04X img fw vers:0x%04X\n", __func__,
			fw_ver_ic, fw->fw_version);
	dev_info(dev, "%s: ic config vers:0x%02X img config vers:0x%02X\n", __func__,
	...
	if (forced) {
		dev_info(dev, "%s: forced upgrade without version checking\n",
					__func__);
		goto do_upgrade;
	}
	...
	dev_info(dev, "%s: upgrade is not required\n", __func__);
	goto exit;
	
do_upgrade:
	rc = cyttsp4_upgrade_firmware(ttsp, fws, 0, 0);
...
static int cyttsp4_load_func(struct cyttsp4_device *ttsp,
		bool forced)
{
	return cyttsp4_fw_upgrade_platform(ttsp, forced);
}
...
printk("%s: ttsp=%p\n", __func__, ttsp);
	...
	cyttsp4_fw_upgrade_platform(ttsp, 0);
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_USE_FW_BIN_FILE
