...
{
	int chargertype = 0;
	int chager_connected = 0;
    
	chargertype = get_cable_type();
	//printk("[TSP] charger type is %d !!!!!!\n",chargertype);
	...
	return chager_connected;   
}

static int cyttsp4_check_charger(struct cyttsp4_core_data *cd)
{
	...
	command_buf[1] = 0x51; // charger mode
	command_buf[2] = 0x01; // param size
	memset(return_buf, 0, sizeof(return_buf));
	rc = cyttsp4_exec_cmd_(cd, CY_MODE_OPERATIONAL, 
						   command_buf, 3, 
						   return_buf, 3, 
						   500);
	if (rc < 0) {
	...
	dev_dbg(cd->dev, "%s: return of get param %02x %02x %02x\n", __func__,
		return_buf[0], return_buf[1], return_buf[2]);

	if (return_buf[2] == CHARGER_CONNECTED())
		goto exit;

	dev_dbg(cd->dev, "%s: charger connected = %d\n", __func__,
			CHARGER_CONNECTED());

	command_buf[0] = CY_CMD_OP_SET_PARAM;
	command_buf[1] = 0x51; // charger mode
	command_buf[2] = 0x01; // param size
	command_buf[3] = CHARGER_CONNECTED() ? 0x01 : 0x00; // param
	memset(return_buf, 0, sizeof(return_buf));
	rc = cyttsp4_exec_cmd_(cd, CY_MODE_OPERATIONAL, 
						   command_buf, 4, 
						   return_buf, 3, 
	...
	dev_dbg(cd->dev, "%s: return of set param %02x %02x %02x\n", __func__,
		return_buf[0], return_buf[1], return_buf[2]);
...
/*	int hw_rev; FIX BUILD ERROR CANE3G */
	...
	get_tsp_vendor(cd, buff); // need to re-read sysinfo after new f/w downloading
