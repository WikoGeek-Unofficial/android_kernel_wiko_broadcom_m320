...
static uint64_t firmware_commit;
...
static const struct firmware_load_info_t bcmtch_binaries_0[] = {

	/* MUST ALWAYS START WITH WILDCARD */
	{BCMTCHWC, BCMTCHWC, "bcmtchfw_REV00_bin", 0,
		BCMTCH_FIRMWARE_FLAGS_COMBI},
};

static const struct firmware_load_info_t bcmtch_binaries_1[] = {

	/* MUST ALWAYS START WITH WILDCARD */
	{BCMTCHWC, BCMTCHWC, "bcmtchfw_REV01_bin", 0,
		BCMTCH_FIRMWARE_FLAGS_COMBI},
...
#ifndef CONFIG_TOUCH_NOTIFY
struct device *sec_touchkey;
#endif
static struct class *tk_class;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (data->regulator_avdd33 == NULL) {
		pr_info("[TouchKey] regulator_avdd33 Not initialised\n");
		return -ENXIO;
	}
	...
	if (input)
		...
		retval = regulator_disable(data->regulator_avdd33);

	if (!retval) {
		key_value = input;
	} else {
		pr_err("[TouchKey] regulator error %d\n", retval);
		return retval;
	}
...
static struct device_attribute bcmtch_cli_attr =
		__ATTR(cli, S_IWUSR, NULL, bcmtch_os_cli);
				...
				const struct combi_entry *p_entry)
{
	...
	while (p_entry[entry_id].length) {
		...
		case BCMTCH_FIRMWARE_FLAGS_POST_BOOT_CONFIGS:
			...
			firmware_commit = bcmtch_data_ptr->fw_signature.commit;
...
static unsigned char bcmtchfw_REV00_bin[] = {
#include "bcmtchfw_REV00_bin.h"
};

static unsigned char bcmtchfw_REV01_bin[] = {
#include "bcmtchfw_REV01_bin.h"
};

struct firmware bcmtchfw_REV00_fw = {
	.data = bcmtchfw_REV00_bin,
	.size = sizeof(bcmtchfw_REV00_bin)
};

struct firmware bcmtchfw_REV01_fw = {
	.data = bcmtchfw_REV01_bin,
	.size = sizeof(bcmtchfw_REV01_bin)
};
				...
				uint32_t fw_flags)
{
	...
	if (strncmp("bcmtchfw_REV00_bin", fw_name, 14) == 0) {
		p_fw = &bcmtchfw_REV00_fw;
		dev_info(bcmtch_data_ptr->p_device,
			"choosing fw bcmtchfw15200_bin fw_size = %d\n",
								p_fw->size);
	} else if (strncmp("bcmtchfw_REV01_bin,", fw_name, 14) == 0) {
		p_fw = &bcmtchfw_REV01_fw;
		dev_info(bcmtch_data_ptr->p_device,
			"choosing fw bcmtchfw15300_bin fw_size = %d\n",
								p_fw->size);
	} else {
		ret_val = -1;
		dev_err(bcmtch_data_ptr->p_device,
			"cannot find a matching firmware\n");
	}
	...
	struct bcmtch_data *bcmtch_data_ptr)
{
	...
	/* Checking for amethyst's COB revision */
	if (bcmtch_data_ptr->platform_data.bcmtch_fw_rev == 1) {
		while (file < ARRAY_SIZE(bcmtch_binaries_1)) {
			/* find matching chip id */
			if (bcmtch_binaries_1[file].chip_id == id) {
				found_chip_id = true;

				/* if chip id found find matching chip rev */
				if (bcmtch_binaries_1[file].chip_rev == rev) {
					ret_file = file;
					break;
				} else if (bcmtch_binaries_1[file].chip_rev ==
						BCMTCHWC) {
					ret_file = file;
				}
			} else if (bcmtch_binaries_1[file].chip_id == BCMTCHWC) {
				if (!found_chip_id)
					ret_file = file;
			}
		file++;
		...
		while (file < ARRAY_SIZE(bcmtch_binaries_0)) {
			/* find matching chip id */
			if (bcmtch_binaries_0[file].chip_id == id) {
				found_chip_id = true;

				/* if chip id found find matching chip rev */
				if (bcmtch_binaries_0[file].chip_rev == rev) {
					ret_file = file;
					break;
				} else if (bcmtch_binaries_0[file].chip_rev ==
						BCMTCHWC) {
					ret_file = file;
				}
			} else if (bcmtch_binaries_0[file].chip_id == BCMTCHWC) {
				if (!found_chip_id)
					ret_file = file;
			}
		file++;
	...
	if (bcmtch_firmware) {
		...
		if (bcmtch_data_ptr->platform_data.bcmtch_fw_rev == 1) {
			if (bin_file >= 0) {
				...
				bcmtch_dev_download_firmware(
					bcmtch_data_ptr,
					bcmtch_binaries_1[bin_file].filename,
					bcmtch_binaries_1[bin_file].addr,
					bcmtch_binaries_1[bin_file].flags);
			} else
				ret_val = bin_file;
		} else {
			if (bin_file >= 0) {
				...
				bcmtch_dev_download_firmware(
					bcmtch_data_ptr,
					bcmtch_binaries_0[bin_file].filename,
					bcmtch_binaries_0[bin_file].addr,
					bcmtch_binaries_0[bin_file].flags);
			} else
				ret_val = bin_file;
			}
	}
...
static int bcmtch_dev_init_platform(struct device *p_device)
{
	...
	if (p_device) {
		...
		if (of_ret_val) {
			dev_info(p_device,
				"DT property bcmtch-fw-rev not found!\n");
			local_bcmtch_data_p->platform_data
				.bcmtch_fw_rev = 1;
		}

		/* Reverse x-axis orientation for EDN01x (CoB) */
		if (local_bcmtch_data_p->platform_data.bcmtch_fw_rev == 1)
			bcmtch_boot_flag |= BCMTCH_BF_AXIS_X_REVERSE;
...
nodt:
		...
		local_bcmtch_data_p->platform_data.bcmtch_fw_rev =
			p_platform_data->bcmtch_fw_rev;
...
static const struct of_device_id bcmtch15xxx_of_match[] = {
	{.compatible = "bcm,bcmtch15xxx",},
	{},
};

MODULE_DEVICE_TABLE(of, bcmtch15xxx_of_match);
...
static struct i2c_driver bcmtch_i2c_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
		   .of_match_table =  bcmtch15xxx_of_match,
		   },
...
static ssize_t bcmtch_upgrade_fw_show(struct kobject *kobj,
				   struct kobj_attribute *attr, char *buf)
{

	return snprintf(buf, 64, "commit:%016llx\n", firmware_commit);
}

static struct kobj_attribute bcmtch_upgrade_fw_attr = {
	.attr = {
		.name = "upgradefw",
		.mode = 0644,
	},
	.show = &bcmtch_upgrade_fw_show,
};

static struct attribute *bcmtch_properties_attrs[] = {
	&bcmtch_upgrade_fw_attr.attr,
	NULL
};

static struct attribute_group bcmtch_properties_attr_group = {
	.attrs = bcmtch_properties_attrs,
};
					...
					const struct i2c_device_id *id)
{
	...
	*/
	...
	dev_dbg(bcmtch_data_ptr->p_device,
		...
		BCMTCH_DF_INFO);
#ifndef CONFIG_TOUCH_NOTIFY
	sec_touchkey = device_create(tk_class, NULL, 0,
				bcmtch_data_ptr, "touchkey_led");
	...
	if (properties_kobj)
		ret_val = sysfs_create_group(properties_kobj,
					&bcmtch_properties_attr_group);
	if (!properties_kobj || ret_val)
		dev_err(&p_i2c_client->dev, "failed to create board_properties\n");
	return 0;
...
static int bcmtch_i2c_remove(struct i2c_client *p_i2c_client)
{
	...
	sysfs_remove_group(properties_kobj, &bcmtch_properties_attr_group);
...
{
	tk_class = class_create(THIS_MODULE, "tk");
