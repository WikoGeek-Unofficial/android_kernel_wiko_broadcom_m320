...
{
	input_mt_init_slots(input, max_tchs, 0);
	return input_register_device(input);
}

void cyttsp4_init_function_ptrs_heat(struct cyttsp4_mt_data *md)
{
...
static int __init cyttsp4_mt_init(void)
{
	...
	cyttsp4_mt_driver_heat.driver.owner = THIS_MODULE;
	rc = cyttsp4_register_driver_heat(&cyttsp4_mt_driver_heat);
...
{
	cyttsp4_unregister_driver_heat(&cyttsp4_mt_driver_heat);
