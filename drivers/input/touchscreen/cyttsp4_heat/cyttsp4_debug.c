...
static int __init cyttsp4_debug_init(void)
{
	...
	for (i = 0; i < num_core_ids; i++) {
		...
		rc = cyttsp4_register_device_heat(&cyttsp4_debug_infos[i]);
		if (rc < 0) {
	...
	rc = cyttsp4_register_driver_heat(&cyttsp4_debug_driver);
	if (rc) {
...
fail_unregister_devices:
	for (i--; i <= 0; i--) {
		cyttsp4_unregister_device_heat(cyttsp4_debug_infos[i].name,
...
static void __exit cyttsp4_debug_exit(void)
{
	...
	cyttsp4_unregister_driver_heat(&cyttsp4_debug_driver);
	for (i = 0; i < num_core_ids; i++) {
		cyttsp4_unregister_device_heat(cyttsp4_debug_infos[i].name,
			cyttsp4_debug_infos[i].core_id);
