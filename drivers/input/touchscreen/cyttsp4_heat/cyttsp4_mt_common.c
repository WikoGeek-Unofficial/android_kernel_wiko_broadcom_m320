		...
		if (sig != CY_IGNORE_VALUE) {
			...
			if (t > 2) {
				...
				if (tch.abs[CY_TCH_E] == CY_EV_TOUCHDOWN)
					dev_info(dev,
					"%s:tcn=[%d] id=%d e=%d PRESS x=%d,\
					y=%d\n", __func__, num_cur_tch,  t,
					tch.abs[CY_TCH_E], tch.abs[CY_TCH_X],
					tch.abs[CY_TCH_Y]);
				else
				if (tch.abs[CY_TCH_E] == CY_EV_LIFTOFF)
					dev_info(dev,
					"%s: tcn=[%d] id=%d e=%d RELEASE x=%d,\
					y=%d \n", __func__, num_cur_tch,  t,
					tch.abs[CY_TCH_E], tch.abs[CY_TCH_X],
					tch.abs[CY_TCH_Y]);
			#endif
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				dev_info(dev,
				"%s: tcn=[%d] id=%d e=%d PRESS x=%d, y=%d\n",
				__func__, num_cur_tch,  t, tch.abs[CY_TCH_E],
...
static int cyttsp4_mt_probe(struct cyttsp4_device *ttsp)
{
	...
	cyttsp4_init_function_ptrs_heat(md);
...
struct cyttsp4_driver cyttsp4_mt_driver_heat = {
