...
struct bus_type cyttsp4_bus_type_heat;
		...
		struct cyttsp4_core *core)
{
	...
	BUG_ON(bus_find_device(&cyttsp4_bus_type_heat, NULL, &pdev->dev,
			cyttsp4_match_dev));

	pdev->core = core;
	pdev->dev.parent = get_device(&core->dev);
	pdev->dev.bus = &cyttsp4_bus_type_heat;
...
static void _cyttsp4_unregister_dev(struct cyttsp4_device *pdev)
{
	...
	if (!bus_find_device(&cyttsp4_bus_type_heat, NULL, &pdev->dev,
			cyttsp4_match_dev))
		return;
...
int cyttsp4_register_device_heat(struct cyttsp4_device_info const *dev_info)
{
	struct cyttsp4_device *dev;
	struct cyttsp4_core *core;
	int ret;
...
EXPORT_SYMBOL_GPL(cyttsp4_register_device_heat);

int cyttsp4_unregister_device_heat(char const *name, char const *core_id)
{
	struct cyttsp4_device *dev;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(cyttsp4_unregister_device_heat);

int cyttsp4_register_core_device_heat(struct cyttsp4_core_info const *core_info)
{
	struct cyttsp4_core *core;
	struct cyttsp4_adapter *adap;
	int ret;
...
EXPORT_SYMBOL_GPL(cyttsp4_register_core_device_heat);

int cyttsp4_add_adapter_heat(char const *id, struct cyttsp4_ops const *ops,
...
EXPORT_SYMBOL_GPL(cyttsp4_add_adapter_heat);

int cyttsp4_del_adapter_heat(char const *id)
{
	int rc = 0;
	struct cyttsp4_adapter *adap;
	struct cyttsp4_core *core;
...
EXPORT_SYMBOL_GPL(cyttsp4_del_adapter_heat);
...
EXPORT_SYMBOL_GPL(cyttsp4_bus_type_heat);
...
int cyttsp4_register_driver_heat(struct cyttsp4_driver *drv)
{
	int ret;

	drv->driver.bus = &cyttsp4_bus_type_heat;
	if (drv->probe)
...
EXPORT_SYMBOL_GPL(cyttsp4_register_driver_heat);

int cyttsp4_register_core_driver_heat(struct cyttsp4_core_driver *drv)
{
	int ret;
...
EXPORT_SYMBOL_GPL(cyttsp4_register_core_driver_heat);

void cyttsp4_unregister_driver_heat(struct cyttsp4_driver *drv)
{
	driver_unregister(&drv->driver);
}
EXPORT_SYMBOL_GPL(cyttsp4_unregister_driver_heat);

void cyttsp4_unregister_core_driver_heat(struct cyttsp4_core_driver *drv)
{
	driver_unregister(&drv->driver);
}
EXPORT_SYMBOL_GPL(cyttsp4_unregister_core_driver_heat);
...
static int __init cyttsp4_bus_init(void)
{
	...
	error =  bus_register(&cyttsp4_bus_type_heat);
	if (error)
		pr_err("%s: error %d\n", __func__, error);
	else
		pr_debug("%s: ok\n", __func__);
