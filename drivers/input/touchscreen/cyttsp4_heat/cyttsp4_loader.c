...
/* #if defined(CONFIG_MACH_AFYONLTE) */
	if (fw->hw_version >= hw_ver_ic) {
		dev_info(dev,
			"hunny : %s: hw version is newer, will NOT upgrade\n",
			__func__);
#endif
	dev_info(dev, "[TSP] fw_version : %d\n", fw->fw_version);
	dev_info(dev, "[TSP] fw_ver_ic : %d\n", fw_ver_ic);
		if (fw->fw_version > fw_ver_ic) {
			dev_dbg(dev, "%s: Fw is newer, will upgrade\n",
					__func__);
			dev_dbg(dev, "enter here");
			return 1;
		}
	dev_info(dev, "[TSP] %s : %d\n", __func__, __LINE__);
		if (fw->config_version > config_ver_ic) {
			dev_dbg(dev, "%s: Config is newer, will upgrade\n",
					__func__);
			dev_dbg(dev, "enter here");
			return 1;
		}
		...
		struct cyttsp4_touch_firmware *fw)
{
	...
	if (!(data->si->si_ptrs.test->post_codel &
		...
		dev_dbg(dev, "%s: Config CRC invalid, will upgrade\n",
			__func__);
...
//		rc = -EINVAL;
	...
	dev_dbg(dev, "%s: Baseline init success\n", __func__);
...
static void cyttsp4_fw_upgrade(struct cyttsp4_device *ttsp)
{
	...
	if (fw->hw_version != hw_version) {
		cyttsp4_upgrade_firmware(ttsp, fw->img, fw->size);
		goto exit;
	}
#endif
check_upgrade:
	upgrade_from_platform_firmware(ttsp, fw);
...
printk("%s: ttsp=%p\n", __func__, ttsp);
	...
	cyttsp4_set_loader(ttsp, (cyttsp4_loader_func) cyttsp4_load_func);

	init_completion(&data->startup_complete);

	cyttsp4_fw_upgrade(ttsp);
#ifdef FORCE_CAL_IN_PROBE
	cyttsp4_fw_calibrate(ttsp);
#endif
...
static void __init cyttsp4_loader_init_async
			(void *unused, async_cookie_t cookie)
{
	int rc = 0;
	int i, j;
	...
	for (i = 0; i < num_core_ids; i++) {
		...
		rc = cyttsp4_register_device_heat(&cyttsp4_loader_infos[i]);
		if (rc < 0) {
	...
	rc = cyttsp4_register_driver_heat(&cyttsp4_loader_driver);
	if (rc) {
...
fail_unregister_devices:
	for (i--; i >= 0; i--) {
		cyttsp4_unregister_device_heat(cyttsp4_loader_infos[i].name,
...
static int __init cyttsp4_loader_init(void)
{
	...
	async_schedule(cyttsp4_loader_init_async, NULL);
...
static void __exit cyttsp4_loader_exit(void)
{
	...
	cyttsp4_unregister_driver_heat(&cyttsp4_loader_driver);
	for (i = 0; i < num_core_ids; i++) {
		cyttsp4_unregister_device_heat(cyttsp4_loader_infos[i].name,
			cyttsp4_loader_infos[i].core_id);
		pr_info("%s: Unregistering loader device for core_id: %s\n",
