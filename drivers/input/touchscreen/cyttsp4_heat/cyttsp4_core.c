...
static enum cyttsp4_touch_boost_mode g_cyttsp4_boost_mode;
...
static const char *cy_driver_core_name = CYTTSP4_CORE_NAME;
...
/* static int tsp_debug_option = TSP_DEBUG_OPTION; */
...
static void setting_cpufreq(uint32_t on)
{
	if (on == 0) {
		if (!dfs_started)
			schedule_delayed_work(&delayed_cpufreq_work,
				msecs_to_jiffies(1000));
	} else if (on == 1) {
		cancel_delayed_work_sync(&delayed_cpufreq_work);
		if (dfs_started) {
			int ret = 0;
			ret = cpufreq_add_lmt_req(&cpufreq_node,
				"touch_boost", FREQ_MAX, FREQ_MIN_LIMIT);
			pr_info("%s(): cpufreq_add_lmt_req\n",
				__func__);
			if (ret)
				pr_info("%s(): error<%d>! cpufreq_add_lmt_req\n",
					__func__, ret);
			else
				dfs_started = 0;
		}
	} else
		pr_info("%s(): error! setting_cpufreq\n", __func__);
}

void cyttsp4_set_touch_boost_mode(enum cyttsp4_touch_boost_mode mode)
{
	g_cyttsp4_boost_mode = mode;
}
EXPORT_SYMBOL(cyttsp4_set_touch_boost_mode);

void cyttsp4_do_boost_up(void)
{
	switch (g_cyttsp4_boost_mode) {
	case BOOST_NONE:
		break;
	case BOOST_ENABLE:
		setting_cpufreq(1);
...
#endif /* #if defined(TOUCH_BOOSTER_SHMOBILE) */
...
struct device *sec_touchkey_heat;
...
static DEVICE_ATTR(tsp_config_version, S_IRUGO,
		   set_config_version_read_show, NULL);
 /*PART*/			/* TSP config last modifying date */
static DEVICE_ATTR(tsp_touchtype, S_IRUGO,
...
#endif
// KEVI added + 2013.05.03
static DEVICE_ATTR(tsp_calibration, S_IRUGO, tsp_calibration_run, NULL);
...
static DEVICE_ATTR(brightness, S_IWUSR, NULL, touch_led_control);
#endif /* NEED_ALL_TSP_FUNC */
...
static void get_fw_ver_bin(void *device_data)
{
	...
	if (si->ready && si->si_ptrs.samsung_data) {
		...
		u16 siliconID;
		siliconID =
			get_unaligned_be16(&si->si_ptrs.cydata->jtag_si_id3);
		if (siliconID < 0x0250)
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s",
			siliconID < 0x0250 ? "TMA445-B" : "TMA445-D");
	set_cmd_result(ts, buff, strnlen(buff, sizeof(buff)));
	ts->cmd_state = 2;
	dev_dbg(ts->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static int check_rx_tx_num(void *device_data)
{
	...
	if (ts->cmd_param[0] < 0 ||
		...
		dev_dbg(ts->dev, "%s: parameter error: %u,%u\n",
...
check_button_num(void *device_data)
{
	...
	if (ts->cmd_param[0] < 0 ||
		...
		dev_dbg(ts->dev, "%s: parameter error: %u\n",
				__func__, ts->cmd_param[0]);
		node = -1;
		return node;
	}
...
#else
	snprintf(buff, sizeof(buff), "%d", global_idac);
#endif
...
static void fw_update(void *device_data) //item2
{
	...
	if (!loader || !loader_func)
		...
		if (cd->cmd_param[0])
			dev_dbg(cd->dev, "%s: fw update forced\n", __func__);
				  ...
				  *devattr, const char *buf, size_t count)
{
	...
	dev_dbg(ts->dev, "cmd = %s\n", tsp_cmd_ptr->cmd_name);
	for (i = 0; i < param_cnt; i++)
		dev_dbg(ts->dev, "cmd param %d= %d\n", i,
							ts->cmd_param[i]);
		...
		struct device_attribute *devattr, char *buf)
{
	...
	dev_dbg(ts->dev, "tsp cmd: status:%d\n",
			ts->cmd_state);
				    ...
				    *devattr, char *buf)
{
	...
	dev_dbg(ts->dev, "tsp cmd: result: %s\n", ts->cmd_result);
...
static DEVICE_ATTR(cmd, 0777, NULL, store_cmd);
...
#if defined(TOUCH_BOOSTER_SHMOBILE)
	cyttsp4_set_touch_boost_mode(BOOST_NONE);
#endif
...
static struct attribute *sec_tsp_attributes[] = {
	...
	&dev_attr_tsp_config_version.attr,
	&dev_attr_tsp_touchtype.attr,
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	dev_set_drvdata(sec_touchscreen, cd);
...
static int cyttsp4_core_release(struct cyttsp4_core *core)
{
	...
	dev_set_drvdata(sec_touchscreen, NULL);
...
struct cyttsp4_core_driver cyttsp4_core_driver_heat = {
...
static int __init cyttsp4_core_init(void)
{
	...
	rc = cyttsp4_register_core_driver_heat(&cyttsp4_core_driver_heat);
...
{
	cyttsp4_unregister_core_driver_heat(&cyttsp4_core_driver_heat);
