	...
	const struct i2c_device_id *i2c_id)
{
	...
	int_gpio = of_get_gpio(client->dev.of_node, 0);

	rc = devm_gpio_request_one(&client->dev, int_gpio,
		GPIOF_IN, "touch_gpio");
	if (rc) {
		dev_err(&client->dev, "failed to get GPIO %d\n", int_gpio);
		return rc;
	}

	cyttsp4_enable_gpio = of_get_gpio(client->dev.of_node, 1);
	gpio_request(cyttsp4_enable_gpio, "Touchscreen enable");
	gpio_direction_output(cyttsp4_enable_gpio, 1);

	cyttsp4_enable_gpio_rev00 = of_get_gpio(client->dev.of_node, 2);
	gpio_request(cyttsp4_enable_gpio_rev00, "Touchscreen enable");
	...
	rc = cyttsp4_add_adapter_heat(id, &ops, dev);
	if (rc) {
...
int __init cyttsp4_i2c_init(void)
{
	int rc = 0;

/* #if defined(CONFIG_MACH_AFYONLTE) */
#if 0 // TODO : disabled the below code because of linker error. by ungsin */
...
/* #endif */
	...
	if (np) {
		...
		pr_info("%s cypress touchscreen node not found\n", __func__);
		return 0;
	}
...
EXPORT_SYMBOL(cyttsp4_i2c_init);
