...
static unsigned char CTPM_FW[] = {
	...
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
/* #include "FT5436I_20130704_app.i"*/
};
...
int fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	pr_warn("[FTS] (5) fts_ctpm_auto_clb()\n");
...
int ft5x36_read_project_code(struct i2c_client *client, char *project_code)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
	u32  j;
	...
	for (j = 0; j < 33; j++) {
		...
		ft5x06_i2c_read(client, packet_buf, sizeof(packet_buf),
			project_code+j, 1);
		if (*(project_code+j) == '\0')
			break;
	}
...
int  fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	/*pr_warn("[FTS] fw_filenth %d\n",fw_filenth);*/
	struct upgrade_info upgradeinfo;

	pr_warn("[FTS] (3) fts_ctpm_fw_upgrade()\n");
	fts_get_upgrade_info(&upgradeinfo);

	if (*(pbt_buf+dw_lenth-12) == 30) {
		is_5336_fwsize_30 = 1;
		pr_warn("[FTS] *(pbt_buf+dw_lenth-12)= %d\n",
				*(pbt_buf+dw_lenth-12));
	} else {
		is_5336_fwsize_30 = 0;
		pr_warn("[FTS] *(pbt_buf+dw_lenth-12)= %d\n",
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		pr_warn("[FTS] Step 3:check READ-ID\n");
		/*********Step 3:check READ-ID***********************/
		msleep(upgradeinfo.delay_readid);
		auc_i2c_write_buf[0] = 0x90;
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] =
		...
		if (reg_val[0] == upgradeinfo.upgrade_id_1
			&& reg_val[1] == upgradeinfo.upgrade_id_2) {
			pr_warn("[FTS] Step 3: CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
					reg_val[0], reg_val[1]);
			dev_dbg(&client->dev,
	...
	if (is_5336_fwsize_30) {
		...
		ft5x06_i2c_write(client, auc_i2c_write_buf, 1);
		msleep(50);
		pr_warn("[FTS] the erase cmd is 0x63\n");
	} else {
		auc_i2c_write_buf[0] = 0x61;
		/*erase app area*/
		ft5x06_i2c_write(client, auc_i2c_write_buf, 1);
		msleep(FT_UPGRADE_EARSE_DELAY);
		pr_warn("[FTS] the erase cmd is 0x61\n");
	}
	...
	for (j = 0; j < packet_number; j++) {
		...
		if ((((j+1) * FTS_PACKET_LENGTH)%1024) == 0)
			pr_warn("[FTS] write bytes:0x%04x\n",
	...
	if (is_5336_new_bootloader == BL_VERSION_LZ4 ||
		...
		for (i = 0; i < 6; i++) {
			if (is_5336_new_bootloader == BL_VERSION_Z7 &&
				...
				temp = 0x7bfa + i;
				pr_warn("[FTS] the begain adress is 0x7bfa\n");
			} else if (is_5336_new_bootloader == BL_VERSION_LZ4) {
				temp = 0x6ffa + i;
				pr_warn("[FTS] the begain adress is 0x6ffa\n");
			}
			packet_buf[2] = (u8)(temp>>8);
			packet_buf[3] = (u8)temp;
			temp = 1;
		...
		pr_warn("[FTS] send the last six byte\n");
	} else if (is_5336_new_bootloader == BL_VERSION_GZF) {
		for (i = 0; i < 12; i++) {
			if (is_5336_fwsize_30 && DEVICE_IC_TYPE == IC_FT5X36) {
				temp = 0x7ff4 + i;
				pr_warn("[FTS] the begain adress is 0x7ff4\n");
			} else if (DEVICE_IC_TYPE == IC_FT5X36) {
				temp = 0x7bf4 + i;
				pr_warn("[FTS] the begain adress is 0x7bf4\n");
			}
			packet_buf[2] = (u8)(temp>>8);
			packet_buf[3] = (u8)temp;
			temp = 1;
			...
			pr_warn("[FTS] send the last twelve byte %X\n",
					packet_buf[6]);
		}
		pr_warn("[FTS] send the last twelve byte\n");
	}
		...
		unsigned char *firmware_buf)
{
	...
	mm_segment_t old_fs;
	pr_warn("[FTS] (2.2) ft5x0x_ReadFirmware() firmware_name=%s\n",
			firmware_name);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s", firmware_name);
	if (NULL == pfile)
	...
	if (IS_ERR(pfile)) {
		pr_err("error occured while opening file %s.\n", filepath);
		pr_warn("[FTS] error occured while opening file %s.\n",
				filepath);
		return -EIO;
		}
	inode = pfile->f_dentry->d_inode;
	...
	char *firmware_name)
{
	...
	pr_warn("[FTS] (2.1) %s fwsize =%d\n", __func__,
			fwsize);
	if (fwsize <= 0) {
		dev_err(&client->dev, "[FTS] %s ERROR:Get firmware size failed\n",
				__func__);
		pr_warn("[FTS] %s ERROR:Get firmware size failed\n",
				__func__);
		return -EIO;
	}
	...
	if (ft5x0x_ReadFirmware(firmware_name, pbt_buf)) {
		pr_err("[FTS] (2.2.1) error at ft5x0x_ReadFirmware()\n");
	...
	if ((pbt_buf[fwsize-8]^pbt_buf[fwsize-6]) == 0xFF
		...
		if (i_ret != 0) {
			dev_err(&client->dev,
					...
					__func__,  i_ret);
			pr_err("[FTS] %s() - ERROR:[FTS] upgrade failed i_ret = %d.\n",
					__func__,  i_ret);
		} else {
			pr_warn("[FTS] (4) fts_ctpm_auto_clb()\n");
			fts_ctpm_auto_clb(client);  /*start auto CLB*/
		}
		kfree(pbt_buf);
	} else {
		dev_dbg(&client->dev, "FW format error\n");
		pr_warn("FW format error\n");
		kfree(pbt_buf);
		return -EIO;
	}
	return i_ret;
				...
				struct device_attribute *attr, char *buf)
{
	...
	else { /*Update for FIH FTM test*/
		num_read_chars = snprintf(buf, PAGE_SIZE, "FT5336-%02X\n",
			fwver);
		pr_warn("[FTS] ft5x0x_tpfwver_show() version: FT5336-%02x\n",
				fwver);
	}
						...
						const char *buf, size_t count)
{
	...
	retval = kstrtoul(valbuf, 16, (unsigned long *)&wmreg);
	if (0 != retval) {
		dev_err(dev,
			"%s() - ERROR: Could not convert the given input to a number.\n",
				__func__);
		dev_err(dev, "ERROR: The given input was: \"%s\"\n", buf);
		goto error_return;
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (ft5x36_read_project_code(client, projectcode) < 0)
		num_read_chars =
			snprintf(buf, PAGE_SIZE, "get projcet code fail!\n");
	else
		num_read_chars =
			snprintf(buf, PAGE_SIZE, "projcet code = %s\n",
			projectcode);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ft5x0x_get_testparam_from_ini(cfgname) < 0)
		pr_warn("[FTS] get testparam from ini failure\n");
	else {
		#if 1
		if (true == start_test_tp())
			pr_warn("[FTS] tp test pass\n");
		else
			pr_warn("[FTS] tp test failure\n");
		#endif
...
static DEVICE_ATTR(ftsgetprojectcode, S_IRUGO|S_IWUSR,
	ft5x0x_ftsgetprojectcode_show, ft5x0x_ftsgetprojectcode_store);

#ifdef MCAP_TEST
/*static DEVICE_ATTR(ftsi2chidupgrade, S_IRUGO|S_IWUSR,
	ft5x0x_ftsi2chidupgrade_show, ft5x0x_ftsi2chidupgrade_store);*/
...
{
	int err;
	pr_warn("[FTS] sysfs_create_group()\n");
	err = sysfs_create_group(&client->dev.kobj, &ft5x0x_attribute_group);
	if (0 != err) {
		dev_err(&client->dev,
				"%s() - ERROR: sysfs_create_group() failed.error code: %d\n",
				__func__, err);
		pr_warn("[FTS] %s() - ERROR: sysfs_create_group() failed.error code: %d\n",
				__func__, err);
		...
		mutex_init(&g_device_mutex);
			dev_dbg(&client->dev,
					...
					__func__);
			pr_warn("[FTS] ft5x0x:%s() - sysfs_create_group() succeeded.\n",
					__func__);
	}
	return err;
}
...
#if 0 /*DO NOT SUPPORT debug channel*/
...
static const struct file_operations ft5x0x_debug_fops = {
		.owner = THIS_MODULE,
...
{
	/*!!!create_proc_entry is out-of-date!!!*/
	/*ft5x0x_proc_entry = create_proc_entry(PROC_NAME, 0777, NULL);*/
	ft5x0x_proc_entry = proc_create(PROC_NAME,
		0777, NULL, &ft5x0x_debug_fops);
