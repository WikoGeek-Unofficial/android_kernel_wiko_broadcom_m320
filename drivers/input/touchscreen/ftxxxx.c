...
#endif
//++ vitrual key report  custom_t:peter
//#define  VKEY_REPORT_VALUE
//-- vitrual key report  custom_t:peter
...
//++ custom_t peter
int ftxxxx_fw_id = 0,ftxxxx_fw_ver = 0;
...
//++custom_t peter
#define JieMian_FT6X06_VENDOR_ID 0x8b
//--custom_t peter
...
//++ custom_t peter firmware path
 char firmware_name[] = "/system/etc/ft.bin";
//-- custom_t peter
 /*
 Error status codes:
 */
...
static int focaltec_fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf,
			  u32 dw_lenth);
static void Ft5306_Hw_Reset(void);
...
//++ custom_t peter
	...
	u8 FW_version = 0;
	int ret;	
	ret=ft520x_read_reg(0xa8, &vendor);
	ftxxxx_fw_id  =vendor;
	printk("tp vendor id: %x\n", vendor);
	ft520x_read_reg(0xa6, &FW_version);
	ftxxxx_fw_ver = FW_version;
	printk("tp fw_version: %x\n", FW_version);
	if(ret<0)
		return 1;
	else
		return vendor;
...
FTS_BYTE fts_ctpm_get_touch_info(struct focaltech_ftxxxx *ts, ST_TOUCH_INFO* pst_touch_info)
{
	FTS_BYTE *p_data_buf;
...
void ReportFingers_for_ftm(struct focaltech_ftxxxx *ts, ST_TOUCH_INFO *touch_info)
{
	...
	for (i = 0; i < FTS_MAX_FINGER; i++) {
		if (cur_info->points[i].state < FINGER_STALE) {
				...
				if(cur_info->points[i].y == 845)	
			{
				input_report_key(ts->input_dev, BTN_TOUCH, 1);
			
				if(cur_info->points[i].x == 70)
				{
					printk(KERN_INFO "KEY_MENU  down	\n");
					
					input_report_key(ts->input_dev, KEY_MENU, 1);
				}
				if(cur_info->points[i].x == 240)
				{
					printk(KERN_INFO "KEY_HOME down\n");
					input_report_key(ts->input_dev, KEY_HOME, 1);
				}
				if(cur_info->points[i].x == 415)
				{
					input_report_key(ts->input_dev, KEY_BACK, 1);
					printk(KERN_INFO "KEY_BACK  down\n");
				}
	...
	if (active_finers){
		...
		for (i = 0; i < FTS_MAX_FINGER; i++) {
				...
				if(cur_info->points[i].y == 845)	
			{
				...
				if(cur_info->points[i].x == 70)
				{
					printk(KERN_INFO "KEY_MENU  up\n");
					
					input_report_key(ts->input_dev, KEY_MENU, 0);
				}
				if(cur_info->points[i].x == 240)
				{
					printk(KERN_INFO "KEY_HOME  up\n");
					input_report_key(ts->input_dev, KEY_HOME, 0);
				}
				if(cur_info->points[i].x == 415)
				{
					input_report_key(ts->input_dev, KEY_BACK, 0);
					printk(KERN_INFO "KEY_BACK  up\n");
				}
					}
		 		else
...
void ReportFingers(struct focaltech_ftxxxx *ts, ST_TOUCH_INFO *touch_info)
{
	struct FingersQueue * cur_info, *last_info;
	int i, j, reported=0, active_finers = 0;
...
{
	int ret = 0;
	struct focaltech_ftxxxx *ts = container_of(work,
					struct focaltech_ftxxxx, work);
...
//++custom_t peter
		if(bootmode  == 1)			
			ReportFingers_for_ftm(ts, &ft5306_touch_info);
		else
...
{
	struct focaltech_ftxxxx *ts = dev_id;
	queue_work(focaltech_wq, &ts->work);

	return IRQ_HANDLED;
}

static void focaltech_ft5306_enable(struct focaltech_ftxxxx *ts)
{
...
static void focaltech_ft5306_disable(struct focaltech_ftxxxx *ts)
{
	printk("%s: %s()\n", ts->client->name, __func__);
...
{
	struct focaltech_ftxxxx *ts = dev_get_drvdata(dev);
...
//DEV_ATTR(focaltech_ftxxxx, enable, 0664);
...
static void focaltech_fts_firmware_update_func(struct work_struct *work)
{
	update_complete = 0;
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	//++ custom_t peter :after upgrade,reflash fw & id 
	fts_ctpm_get_vendor_id();
	//-- custom_t peter
				   ...
				   struct kobj_attribute *attr, char *buf)
{
	...
	//++ custom_t peter 
	else if  (vendor_id == JieMian_FT6X06_VENDOR_ID)
		return   sprintf(buf, "%s\n", "JieMian FocalTech FT6X06 ");
	//-- custom_t peter
	else
		return   sprintf(buf, "%s\n",  "UNKNOWN");
...
static struct kobj_attribute ft5306_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.FocalTech-Ft5306",
		.mode = S_IRUGO,
	},
		...
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	//++ custom_t peter
	if (ftxxxx_fw_id == 0x53)
		return   sprintf(buf, "%s\n",  "Mutto Optronics") ;
	else if (ftxxxx_fw_id == 0x5D)
		return   sprintf(buf, "%s\n", "BAOMING OPTRONICS CO,.LTD") ;
	else if  (ftxxxx_fw_id == HAWAII_GARNET_FT5X06_VENDOR_ID)
		return sprintf(buf, "%s\n", "FocalTech FT5X06 Hawaii Garnet");
	else if  (ftxxxx_fw_id == G5_A18_FT5X06_VENDOR_ID)
		return   sprintf(buf, "%s\n", "FocalTech FT5X06 G5 A18");
	else if  (ftxxxx_fw_id == G5_A21_FT5316_VENDOR_ID)
		return   sprintf(buf, "%s\n", "FocalTech FT5316 G5 A21");
	else if  (ftxxxx_fw_id == KTOUCH_W68_FT6X06_VENDOR_ID)
		return   sprintf(buf, "%s\n", "FocalTech FT6X06 Ktouch W68");
	else if  (ftxxxx_fw_id == JieMian_FT6X06_VENDOR_ID)
		return   sprintf(buf, "%s%x\n", "JieMian-L4020AP-FT6X06-",ftxxxx_fw_ver);   // JSTINNO_TAO.PAN, DATE20140702, NOTE: Bug DACA-871 LINE
	else
		return   sprintf(buf, "%s\n",  "UNKNOWN");
	//--custom_t peter
	ret = strlen(buf) + 1;
...
static DEVICE_ATTR(vendor, S_IRUGO, android_touch_vendor_show, NULL);

static ssize_t android_touch_fw_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	//++custom_t peter
...
static DEVICE_ATTR(fw_id, S_IRUGO, android_touch_fw_id_show, NULL);


static ssize_t android_touch_fw_ver_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	//++ custom_t peter
	//int ver = ftxxxx_fw_ver;
...
static DEVICE_ATTR(fw_ver, S_IRUGO, android_touch_fw_ver_show, NULL);
...
{	
	...
	printk("-----%x,%x------\n",pbt_buf[fwsize -1],pbt_buf[fwsize -2]);
	version = pbt_buf[fwsize -2];
	vfree(pbt_buf);
	return version;
}
...
static int android_touch_sysfs_init(void)
{
	int ret ;
...
static void android_sysfs_deinit(void)
{
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts = kzalloc(sizeof(struct focaltech_ftxxxx), GFP_KERNEL);
	ts->client = client;
	i2c_set_clientdata(client, ts);
	printk(KERN_ERR "probing for focaltech device %s at $%02X...\n", client->name, client->addr);
	...
	if (pdata) {
		...
		if (!of_property_read_u32(np, "gpio-irq-pin", &val))
			ts_gpio_irq_pin = val;
		printk(KERN_INFO "lht---%s---%d\n", __func__, ts_gpio_irq_pin);
...
//-- vitrual key report  custom_t:peter
	...
	//--custom_t peter
		...
		if (auto_update_fw) {
			...
			wake_lock(&update_wake_lock);
			queue_delayed_work(focaltech_wq, &ft6x06_firmware_update, 5*HZ);
		}
	...
	//++ tp sys custom_t:peter
	if(android_touch_sysfs_init())
		pr_err("failed to create android_touch\n");
	//-- tp sys custom_t:peter
...
//-- vitrual key report  custom_t:peter
	...
	if (!ts->use_irq) {
		printk(KERN_ERR "focaltech  device %s in polling mode\n", client->name);
	}
...
{
struct focaltech_ftxxxx *ts = i2c_get_clientdata(client);
...
static void focaltech_ft5306_early_suspend(struct early_suspend *h)
{
	...
	ts = container_of(h, struct focaltech_ftxxxx, early_suspend);
...
static int focaltech_ft5306_init(void)
{
	...
	printk(KERN_INFO "Focaltech I2C driver init");

	focaltech_wq = create_singlethread_workqueue("focaltech_wq");
	if (!focaltech_wq) {
		printk(KERN_ERR "Could not create work queue focaltech_wq: no memory");
		return -ENOMEM;
	}
