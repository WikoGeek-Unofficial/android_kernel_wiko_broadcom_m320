...
static int cpg_get_freq(struct clk_rate *rates)
{
	...
	if ((get_rate.i_clk < 0) || (get_rate.zg_clk < 0)
		...
		|| (get_rate.zt_clk < 0) || (get_rate.zx_clk < 0)
		|| (get_rate.hp_clk < 0) || (get_rate.zs_clk < 0)
		|| (get_rate.zb_clk < 0) || (get_rate.zb3_clk < 0)) {
		return -EINVAL;
	}
...
static int stop_dfs_cmd(char *para, int size)
{
	...
	FUNC_MSG_IN;
#ifdef CONFIG_SHMOBILE_CPUFREQ
	cpufreq_add_lmt_req(&cpufreq_stop, "pmdbg",
...
static int suppress_cmd(char *para, int size)
{
	...
	if (0 == ret) {
		cpufreq_add_lmt_req(&cpufreq_min_lmt, "pmdbg",
				FREQ_MID_LOWER_LIMIT, FREQ_MIN_LIMIT);
