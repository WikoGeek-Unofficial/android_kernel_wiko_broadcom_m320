...
/* voice call status */
static bool g_sndp_voicecall_status;
/* FSI format */
...
static struct cpufreq_lmt_node cpufreq_min_lmt;
static struct cpufreq_lmt_node cpufreq_stop;
...
/**
 * @brief       Update Character Buffer to get rid of invalid chars before
 *              passing the buffer to kstrtoul() function.
 *
 * @param       none.
 *
 * @retval      none.
 */
static void sndp_update_buffer(unsigned char *pstr, unsigned int size)
{
	unsigned int counter;

	if ((pstr == NULL) || (size < SNDP_PROC_MAX_BUF_SIZE)) {
		sndp_log_err("Invalid Args. pstr=0x%x size=%d\n",
			(unsigned int) pstr, size);
		return;
	}
	...
	struct snd_soc_card *fsi_soc_card)
{
	...
	if (NULL != g_sndp_parent) {
		...
		if (NULL != entry)
			sndp_log_info("Log Level proc entry successful\n");
		 else {
		...
		if (NULL != reg_dump_entry)
			sndp_log_info("proc entry for reg dump succesful\n");
		 else {
		...
		if (NULL != voicecall_debug_entry)
			sndp_log_info("voice call proc entry successful\n");
		else {
			sndp_log_always_err("create_proc_entry(LOG) failed\n");
			goto mkproc_sub_err;
		}
			...
			size_t size, loff_t *ppos)
{
	...
	iLen = snprintf(page, sizeof(page), "0x%08x\n",
			 (int)g_sndp_voicecall_debug_level);
	...
	loff_t *ppos)
{
	...
	if (count > SNDP_PROC_MAX_BUF_SIZE)
		count = SNDP_PROC_MAX_BUF_SIZE;

	proc_buf = (char *)kmalloc(count, GFP_KERNEL);
	if (proc_buf == NULL || buffer == NULL || count <= 0 ||
		count >= SNDP_PROC_MAX_BUF_SIZE) {
		sndp_log_err("Invalid buffer OR size over len[%d].\n", count);
		if (proc_buf != NULL)
			kfree(proc_buf);
		return -EFAULT;
	}
	memset(proc_buf, 0, (unsigned int)count);
	/* copy, from user */
	...
	sndp_update_buffer(proc_buf, SNDP_PROC_MAX_BUF_SIZE);
	ret = kstrtoull((const char *)proc_buf, 0, &uiIn);
	if (ret != 0) {
		sndp_log_err("kstrtoull returned error=%d proc_buf %s\n",
					ret, proc_buf);
		kfree(proc_buf);
		return -EFAULT;
	}
	...
	loff_t *ppos)
{
	...
	sndp_update_buffer(proc_buf, SNDP_PROC_MAX_BUF_SIZE);
	iRet = kstrtoul((const char *)proc_buf, 0, &ulIn);
	if (0 != iRet) {
		sndp_log_err("kstrtoul returned error=%d proc_buf %s\n",
				iRet, proc_buf);
		kfree(proc_buf);
		return iRet;
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	/* voice call stop */
	if ((uiMode == SNDP_MODE_NORMAL) && (g_sndp_voicecall_status == true)
			&& (uiDirection == SNDP_PCM_IN)) {
		/* Wake Lock */
	...
	if (uiProcess & SNDP_PROC_INCOMM_START) {
		if (((SNDP_PCM_OUT == uiDirection) &&
			...
			if (g_dfs_mode_min_flag) {
				sndp_log_info("add cpufreq min limit\n");
				ret = cpufreq_add_lmt_req(&cpufreq_min_lmt,
							"snd-soc-fsi",
							FREQ_MID_LOWER_LIMIT,
							FREQ_MIN_LIMIT);
			} else {
				sndp_log_info("add cpufreq max limit on min\n");
				ret = cpufreq_add_lmt_req(&cpufreq_stop,
							"snd-soc-fsi",
							FREQ_MAX,
							FREQ_MIN_LIMIT);
			}
			if (ERROR_NONE != ret)
				sndp_log_err("cpufreq_add_lmt_req ret[%d]\n",
							ret);
		}
		/* Wake Lock */
		sndp_wake_lock(E_LOCK);
...
static void sndp_work_voice_stop(struct sndp_work_info *work)
{
	...
	if (g_sndp_voicecall_status == false) {
		sndp_log_debug(" Already voice call clean up done !!!\n");
		return;
	}
...
/*	g_sndp_start_call_wait = 0;*/
	...
	if (SNDP_MODE_INCALL != SNDP_GET_MODE_VAL(GET_OLD_VALUE(SNDP_PCM_OUT))) {
		...
		if (g_dfs_mode_min_flag) {
			sndp_log_info("remove cpufreq min limit\n");
			cpufreq_del_lmt_req(&cpufreq_min_lmt);
		} else {
			sndp_log_info("remove cpufreq max limit on min\n");
			cpufreq_del_lmt_req(&cpufreq_stop);
		}
	} else {
