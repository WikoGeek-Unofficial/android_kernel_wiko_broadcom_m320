...
extern int  g_sndp_voicecall_debug_level;
/*
 * Register Table Definisions
 */

/* Added for FM */
int mdbar_mix0_vol = 0x00000182;
bool fmmute_flag = false;
bool call_flag;
bool isScuwstarted = false;
/* Added for FM */

#define MIN_DB_VOL 0x000003FF

static unsigned int sidevol = MIN_DB_VOL;
...
/*        Reg			Val		D  C */
	...
	{ SCUW_SEL_SELCR0,	0x00000000,	0, 0 },
	/* 1: MIX0 output */
	...
	/* Side Tone = UL data */
	{ SCUW_SEL_SELCR16,     0x00000006,     0, 0 },
	/*SEL16 output data*/
	{ SCUW_SEL_SELCR19,     0x00000003,     0, 0 },
	/*   1 : FSI-IF operates. */
...
/*	{ SCUW_FSIIF_FSIIR,	0x00000000,	0, 0 }, */
	...
	/* 0: Use mix mode  */
	{ SCUW_MIXBR_MIX0,	0x00000000,	0, 0 },
	/* 0: step mode  */
	{ SCUW_MIXMR_MIX0,	0x00000000,	0, 0 },
	/*volume level  */
	{ SCUW_MVPDR_MIX0,	0x00000000,	0, 0 },
	{ SCUW_MDBER_MIX0,      0x00000000,     0, 0 },
	/* Mix0 decibel B register Muted */
	{ SCUW_MDBBR_MIX0,      0x000003FF,     0, 0 },
	{ SCUW_MDBCR_MIX0,      0x000003FF,     0, 0 },
	{ SCUW_MDBDR_MIX0,      0x000003FF,     0, 0 },
	{ SCUW_MDBER_MIX0,      0x00000001,     0, 0 },
	/* 0: processing state Mix 0  */
	{ SCUW_MIXIR_MIX0,	0x00000000,	0, 0 },
	/*TDM MUX2 output to MIX0 system B*/
	{ SCUW_SW_TDM,          0x00000000,     0, 0 },
	{ SCUW_SW_SRC0,         0x00000002,     0, 0 },
};
/* Table for Voicecall(PortA) */
static struct common_reg_table scuw_reg_tbl_voicecallA_record_debug[] = {
/*        Reg                   Val             D  C */
        /*   1 : SPU2V output data */
	{ SCUW_SEL_SELCR21,     0x00000001,     0, 0 },
	/*   1 : Voice data(from VOIP) */
...
/*      { SCUW_FSIIF_FSIIR,     0x00000000,     0, 0 }, */
	/* 010 : Channel 1 to 7 are copied Channel 0 */
	{ SCUW_VD_VDSET,        0x00000002,     0, 0 },
/* Voice call record UL + DL data */
	/*   0 : Voice DL data */
	{ SCUW_SEL_SELCR2,      0x00000000,     0, 0 },
	/* 010: SRC1 Module operates */
	{ SCUW_MSTP0,           0x00000002,     0, 0 },
	/*   0 : Resets the SRC0. */
...
/* Table for Voicecall(PortA) */
static struct common_reg_table scuw_reg_tbl_voicecallA_dl_debug[] = {
/*        Reg                   Val             D  C */
	/*   1 : SPU2V output data */
	{ SCUW_SEL_SELCR21,     0x00000001,     0, 0 },
	/*   1 : Voice data(from VOIP) */
...
/*      { SCUW_FSIIF_FSIIR,     0x00000000,     0, 0 }, */
	/* 010 : Channel 1 to 7 are copied Channel 0 */
	{ SCUW_VD_VDSET,        0x00000002,     0, 0 },
/* Voice call DL debugging */
	/* 0 : voice call data */
	{SCUW_SEL_SELCR4,	0x00000000,	0, 0 },
	/* 0: sel 4 data */
	{SCUW_SEL_SELCR8,       0x00000000,     0, 0 },
	/* 0 : sel8 data */
	{SCUW_SEL_SELCR9,       0x00000000,     0, 0 },
	/* 0: sel 9 data */
	{SCUW_SEL_SELCR10,       0x00000000,     0, 0 },
...
/* Table for Voicecall(PortA) */
static struct common_reg_table scuw_reg_tbl_voicecallA_ul_debug[] = {
/*        Reg                   Val             D  C */
	/*   1 : SPU2V output data */
	{ SCUW_SEL_SELCR21,     0x00000001,     0, 0 },
	/*   1 : Voice data(from VOIP) */
...
/*      { SCUW_FSIIF_FSIIR,     0x00000000,     0, 0 }, */
	/* 010 : Channel 1 to 7 are copied Channel 0 */
	{ SCUW_VD_VDSET,        0x00000002,     0, 0 },
/* Voice call UL debugging */
	/*3 : Audio IC UL data */
	{SCUW_SEL_SELCR13,      0x00000003,     0, 0 },
	/* 4: cpuc fifo2 operates */
	{ SCUW_MSTP3,           0x00000004,     0, 0 },
	/*   0 : Reset FIFO. */
	{ SCUW_SWRSR_CF2,       0x00000000,     0, 0 },
...
int scuw_start(const u_int uiValue, const u_int rate)
{
	...
	u_int			tbl_size	= 0;
	bool isFmStart = false;
	int ret;
	sndp_log_debug_func("start\n");

	/* Start SCUW Clock Supply */
	ret = audio_ctrl_func(SNDP_HW_SCUW, STAT_ON, 1);
	if (ERROR_NONE != ret)
		return ret;

	call_flag = ((SNDP_GET_MODE_VAL(uiValue) == SNDP_MODE_INCALL)
			|| (SNDP_GET_MODE_VAL(uiValue) == SNDP_MODE_INCOMM));
	...
	if ((false == (dev & SNDP_BLUETOOTHSCO)) &&
		...
		switch (g_sndp_voicecall_debug_level) {

		case SNDP_VOICECALL_DL_DEBUG:
			reg_tbl  = scuw_reg_tbl_voicecallA_dl_debug;
			tbl_size = ARRAY_SIZE(scuw_reg_tbl_voicecallA_dl_debug);
			sndp_log_debug_func("scuw_reg_tbl_voicecallA_dl_debug\n");
			break;
		case SNDP_VOICECALL_UL_DEBUG:
			reg_tbl  = scuw_reg_tbl_voicecallA_ul_debug;
			tbl_size = ARRAY_SIZE(scuw_reg_tbl_voicecallA_ul_debug);
                        sndp_log_debug_func("scuw_reg_tbl_voicecallA_ul_debug\n");
			break;
		case SNDP_VOICECALL_DL_UL_DEBUG:
			reg_tbl  = scuw_reg_tbl_voicecallA_record_debug;
			tbl_size = ARRAY_SIZE(scuw_reg_tbl_voicecallA_record_debug);
                        sndp_log_debug_func("scuw_reg_tbl_voicecallA_record_debug\n");
			break;
		default:
			reg_tbl  = scuw_reg_tbl_voicecallA;
			tbl_size = ARRAY_SIZE(scuw_reg_tbl_voicecallA);
                        sndp_log_debug_func("scuw_reg_tbl_voicecallA.\
			Voice call debug level is %d\n", g_sndp_voicecall_debug_level);
			break;
		}
	/* BLUETOOTHSCO */
	...
	/*If the scuw starts after the 'Sidetone Attenuation' mixer control*/
	if ((false == (dev & SNDP_BLUETOOTHSCO)) &&
		(false == (dev & SNDP_FM_RADIO_TX)) &&
		(false == (dev & SNDP_FM_RADIO_RX))) {
			sidevar = 1;
			iowrite32(sidevol, g_scuw_Base + SCUW_MDBBR_MIX0);
	}
	/* Added for FM */
	sndp_log_info("scuw_start isFmStart = %d\n", isFmStart);
	if(isFmStart == true) {
		sndp_log_info("Setting volume during start %x, fmmute_flag = %d\n", mdbar_mix0_vol, fmmute_flag);
		scuw_set_fmmute(fmmute_flag);
		scuw_set_fmvolume(mdbar_mix0_vol);
	}
	/* Added for FM */
...
int scuw_stop(void)
{
	...
	/* Reset the register to 0x3FF at the end of the call*/
	if (sidevar) {
		sidevol = MIN_DB_VOL;
		iowrite32(sidevol, g_scuw_Base + SCUW_MDBBR_MIX0);
		sidevar = 0;
	...
	ret = audio_ctrl_func(SNDP_HW_SCUW, STAT_OFF, 1);
	if (ERROR_NONE != ret)
		return ret;
...
void scuw_sidetone_val(int vol)
{
	sndp_log_info("%s : vol = 0x%x isScuwstarted = %d\n",
				__func__, vol, isScuwstarted);
	sidevol = vol;
	if (isScuwstarted == true)
		iowrite32(vol, g_scuw_Base + SCUW_MDBBR_MIX0);
}

int scuw_get_sidetone_val(void)
{
	return sidevol;
}

/* Added for FM */
void scuw_set_fmmute(bool muteflag)
{
	sndp_log_info("set_fmmute isScuwstarted = %d\n", isScuwstarted);
	if(isScuwstarted == true) {
		if ((muteflag == true) && (call_flag == false)) {
			sndp_log_info("FM MUTE LATEST");
			iowrite32(0x30001, g_scuw_Base + SCUW_MIXBR_MIX0);
		}
		else {
			sndp_log_info("FM UNMUTE");
			iowrite32(0x30000, g_scuw_Base + SCUW_MIXBR_MIX0);
		}
	}
	fmmute_flag = muteflag;
	sndp_log_info("updating  fmmute_flag = %d\n", fmmute_flag);
}

void scuw_set_fmvolume(int volume)
{
	sndp_log_info("set_volume isScuwstarted = %d\n", isScuwstarted);
	if(isScuwstarted == true) {
		sndp_log_info("setfmvolume volume = %d\n", volume);
		iowrite32(volume, g_scuw_Base + SCUW_MDBAR_MIX0);
		iowrite32(0x00000000, g_scuw_Base + SCUW_MDBDR_MIX0);
	}
	mdbar_mix0_vol = volume;
	sndp_log_info("updating  mdbar_mix0_vol = %d\n", mdbar_mix0_vol);
}

int scuw_get_fmvolume(void)
{
    sndp_log_info("get_fmvolume returning %d\n", mdbar_mix0_vol);
    return mdbar_mix0_vol;
}
...
void scuw_voice_test_start_a(void)
{
	...
	if (audio_ctrl_func(SNDP_HW_SCUW, STAT_ON, 1) != ERROR_NONE)
		return;
...
void scuw_voice_test_stop_a(void)
{
	...
	if (audio_ctrl_func(SNDP_HW_SCUW, STAT_OFF) != ERROR_NONE)
		return;
}
#endif /* SOUND_TEST */
