...
#ifdef CONFIG_SND_SPK_CHG_PUMP
#include <linux/gpio.h>
#define SPK_CHG_PUMP_GPIO CONFIG_SND_SPK_CHG_PUMP_GPIO
#define SPK_CHG_PUMP_DELAY CONFIG_SND_SPK_CHG_PUMP_DELAY
#define SPK_DRV_PD_TIME 1000
#endif
...
#ifdef CONFIG_SND_SPK_CHG_PUMP
static int d2153_spkpga_event(struct snd_soc_dapm_widget *widget,
			struct snd_kcontrol *kctl, int event)
{
	dlg_info("%s() event = %d\n", __func__, event);
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		gpio_set_value(SPK_CHG_PUMP_GPIO, 1);
		usleep_range(SPK_CHG_PUMP_DELAY, SPK_CHG_PUMP_DELAY + 100);
		break;
	case SND_SOC_DAPM_POST_PMD:
		usleep_range(SPK_DRV_PD_TIME, SPK_DRV_PD_TIME + 100);
		gpio_set_value(SPK_CHG_PUMP_GPIO, 0);
		break;
...
static const struct snd_soc_dapm_widget d2153_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA("Earpiece PGA", D2153_EP_CTRL, D2153_EP_AMP_EN_SHIFT,
			 D2153_NO_INVERT, NULL, 0),
#ifdef CONFIG_SND_SPK_CHG_PUMP
	SND_SOC_DAPM_PGA_E("Speaker PGA", D2153_SP_CTRL, D2153_SP_AMP_EN_SHIFT,
			 D2153_NO_INVERT, NULL, 0, d2153_spkpga_event,
 ...
 */
static int d2153_hw_params(struct snd_pcm_substream *substream,
...
static struct snd_soc_dai_ops d2153_dai_ops = {
	...
	size_t len, loff_t *offset)
{
	...
	if ((buffer == NULL) || (len <= 0)) {
		dlg_err("%s: Input Buffer Argument NULL.\n", __func__);
		return -EFAULT;
	}
...
void d2153_codec_debug_proc_init(struct d2153_codec_priv *d2153_codec)
{
	...
	if (entry == NULL)
		dlg_err("%s:Unable to create codec0 proc entry.\n", __func__);
	else
		dlg_info("%s:Created codec0 proc entry.\n", __func__);
}
