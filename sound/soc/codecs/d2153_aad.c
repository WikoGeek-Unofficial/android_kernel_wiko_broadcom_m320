...
{
	unsigned int size = sizeof(unsigned char) * 4;
...
#if custom_t_BUG_FIX
static void releasekeyifneeded(struct d2153_aad_priv *d2153_aad)
{
	if(d2153_aad == NULL)
		return;
	...
	//if (d2153_aad->button.status == D2153_BUTTON_PRESS) {
		...
		d2153_aad->button.status = D2153_BUTTON_RELEASE;
		dlg_warn("custom_t:%s event Rel key=%d!\n", __func__,
