...
#else
static inline uint32_t sec_hal_dbg_reg_set(uint32_t *dbgreg1, uint32_t *dbgreg2,
					   uint32_t *dbgreg3)
{
	return 1;
}
...
static struct property device_okay = {
	.name = "status",
	.length = sizeof "okay",
	.value = "okay",
};

static char stm_select_prop_buf[4];
static struct property stm_select_prop = {
	.name = "stm-select",
	.length = sizeof stm_select_prop_buf,
	.value = stm_select_prop_buf,
};

static void set_ztr_clk_rate(struct device_node *np)
{
	uint32_t ztr_clk_rate;
	struct clk *ztr_clk = clk_get(NULL, "ztr_clk");

	if (!ztr_clk) {
		pr_err("Failed to get ztr_clk");
		return;
	}

	of_property_read_u32(np, "ztr-clk-rate", &ztr_clk_rate);

	if (clk_set_rate(ztr_clk, ztr_clk_rate) < 0) {
		pr_err("STM Trace: Failed to set ztr_clk_rate ");
		return;
	} else {
		pr_info("STM Trace: ZTR clock rate set to %ld\n",
			clk_get_rate(ztr_clk));
	}
}

/* This must be called before of_platform_populate, as it modifies the DT */
void __init u2evm_init_stm_select(void)
{
...
/* For case that TZ:Secure ISSW or Non-TZ:R-Loader has selected debug mode already! */
  ...
  printk(KERN_ALERT "final stm_select=%d\n", stm_select);
	...
	/* FIRST, MODIFY DT TO ENABLE STM, AND DISABLE CONFLICTING SDHI */
	if (stm_select >= 0) {
		struct device_node *np;

		if (stm_select == 1)
			np = of_find_node_by_path("/sdhi@ee120000");
		else /* stm_select == 0 */
			np = of_find_node_by_path("/sdhi@ee100000");

		if (np)
			of_update_property(np, &device_disabled);
		else
			pr_err("sdhi node not found, so not disabled\n");

		np = of_find_compatible_node(NULL, NULL, "arm,coresight-stm");
		if (np) {
			of_update_property(np, &device_okay);

			/*
			 * Pass final stm-select through to device driver as a
			 * property - this is interpreted as the name of a
			 * pinctrl state for it to select; DT should have
			 * "0" and "1" states.
			 */
			snprintf(stm_select_prop.value, stm_select_prop.length,
					"%d", stm_select);
			of_add_property(np, &stm_select_prop);
			set_ztr_clk_rate(np);
		} else {
			pr_err("stm node not found, so not enabled\n");
		}
	}

  /* SECOND, ENABLE TERMINAL POWER FOR STM CLK AND DATA PINS */
  /* (Better handled through regulator notifiers?) */
  if (stm_select >= 0) {
    __raw_writel(__raw_readl(MSEL3CR) | (1<<28), MSEL3CR);
  }

  /* THIRD, CONFIGURE SYS-(TRACE) FUNNEL-STM, and SYS-TPIU-STM */
  if (stm_select >= 0) {
