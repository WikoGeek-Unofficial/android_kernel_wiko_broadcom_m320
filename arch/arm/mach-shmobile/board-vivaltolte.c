...
static void __init board_init(void)
{
	...
	l2x0_init_later();
#endif
	/* Camera flash will be not used for VivaltoLTE */
	/* add_primary_cam_flash_rt8547(GPIO_PORT99, GPIO_PORT100); */

if (system_rev < BOARD_REV_002)
	 add_primary_cam_flash_rt8547(GPIO_PORT99, GPIO_PORT100);
else
...
#endif
	if (platform_device_register(&wlan_sdio_device) < 0)
		pr_err("%s %s\n", __func__,
			"Unable to register wlan_sdio_device !");
}
