...
#endif
static int boot_on_power_areas;
...
static struct drv_pd_mapping_table tbl2[] = {
	...
	/* MMCIF.0 DT	*/	{ "e6bd0000.mmcif",	ID_A3SP },
	...
	/* DT names for sdhi */
	/* SDHI0	*/	{ "ee100000.sdhi",	ID_A3SP },
	/* SDHI1	*/	{ "ee120000.sdhi",	ID_A3SP },
	/* SDHI2	*/	{ "ee140000.sdhi",	ID_A3SP },
...
static struct pdi_mapping_table pdi_tb2[] = {
	...
	/* MMCIF.0 DT   */      { "e6bd0000.mmcif",     pdi_init_val},
	...
	/* usb1        */       { "usb1",               pdi_init_val},
	/* port1        */      { "port1",              pdi_init_val},
	/* hub          */      { "1-0:1.0",            pdi_init_val},
	...
	/* SDHI0	*/	{ "ee100000.sdhi",	pdi_init_val},
	/* SDHI1	*/	{ "ee120000.sdhi",	pdi_init_val},
	/* SDHI2	*/	{ "ee140000.sdhi",	pdi_init_val},
...
static int power_domain_driver_probe(struct device *dev)
{
	unsigned area = BIT(to_platform_device(dev)->id);

	if (is_power_status_on(area))
		pm_runtime_set_active(dev);
	pm_runtime_enable(dev);
	pm_runtime_irq_safe(dev);
	/*
	 * If an area was already on from the boot loader, hold it on until
	 * late init, giving time for other drivers to do their own
	 * pm_runtime_get calls. Needed in particular for TPU PWM backlights.
	 */
	if (pm_runtime_active(dev)) {
		pm_runtime_get_noresume(dev);
		boot_on_power_areas |= area;
	}
	return 0;
}
...
static void pd_early_suspend(struct early_suspend *h)
{
	cancel_delayed_work_sync(&pdwait_work);
}

static void pd_late_resume(struct early_suspend *h)
{
	queue_delayed_work_on(CPU0_ID, pdwait_wq,
		&pdwait_work, usecs_to_jiffies(DELAYED_TIME_IN_US));
}

static struct early_suspend power_domain_early_suspend = {
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 50,
	.suspend = pd_early_suspend,
	.resume  = pd_late_resume,
};
...
power_a4rm_mask = POWER_A4RM;
	...
	if (shmobile_is_u2())
		register_early_suspend(&power_domain_early_suspend);

	return 0;
}

arch_initcall(power_domain_driver_init);

/*
 * power_domain_init_complete: allow already-on-at-boot areas to shut down
 */
static int __init power_domain_init_complete(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(pd); i++)
		if (boot_on_power_areas & BIT(i)) {
			pr_info("[PDC] Releasing %s after boot\n",
				dev_name(&pd[i]->dev));
			pm_runtime_put_sync_suspend(&pd[i]->dev);
		}

	return 0;
}
late_initcall_sync(power_domain_init_complete);
...
{
	spin_lock(&pdc_lock);
	power_status_set(area, on);
	spin_unlock(&pdc_lock);
}
EXPORT_SYMBOL(power_domain_set_status);
