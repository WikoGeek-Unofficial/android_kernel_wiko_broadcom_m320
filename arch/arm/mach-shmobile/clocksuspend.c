...
#define ZTRFC_MASK	0xF
#define ZTRFC_SHIFT	20
...
static int cpg_set_kick(unsigned int time)
{
	...
	memory_log_dump_int(PM_DUMP_ID_KICK_FREQ, PM_DUMP_START);
	__raw_writel(BIT(31) | __raw_readl(FRQCRB), FRQCRB);
	ret = cpg_wait_kick(wait_time);
	memory_log_dump_int(PM_DUMP_ID_KICK_FREQ, ret);
	return ret;
}
...
int cpg_get_freq(struct clk_rate *rates)
{
	...
	if ((rates->i_clk < 0) || (rates->zg_clk < 0)
		...
		|| (rates->zt_clk < 0) || (rates->zx_clk < 0)
		|| (rates->hp_clk < 0) || (rates->zs_clk < 0)
		|| (rates->zb3_clk < 0)) {
		return -EINVAL;
	}
...
static int cpg_get_freqval(int clk, int *div)
{
	...
	} else if ((clk == Z_CLK) || (clk == ZT_CLK) || (clk == ZX_CLK)
		|| (clk == HP_CLK) || (clk == ZS_CLK)) {
...
static void cpg_dbg_check_div_rates(void)
{
	int clk, div, div_rate;
	unsigned int reg_val;

	reg_val = __raw_readl(FRQCRA);
	for (clk = I_CLK; clk <= M5_CLK; clk++) {
		div_rate = HW_TO_DIV(reg_val, clk);
		div = __match_div_rate(clk, div_rate);
		BUG_ON(div < 0);
	}

	reg_val = __raw_readl(FRQCRB);
	for (clk = Z_CLK; clk <= ZS_CLK; clk++) {
		div_rate = HW_TO_DIV(reg_val, clk);
		div = __match_div_rate(clk, div_rate);
		BUG_ON(div < 0);
	}
...
static int cpg_set_freq(const struct clk_rate rates)
{
	...
	for (i = 0; i < CPG_SET_FREQ_MAX_RETRY; i++) {
		...
		/* change FRQCR(A/B) ? */
		if (frq_change) {
			reg = __raw_readl(FRQCRB);

			/* Don't modify ZTR clock */
			reg &= (ZTRFC_MASK << ZTRFC_SHIFT);
			...
			/* apply setting */
			memory_log_dump_int(PM_DUMP_ID_SET_FRQCRB_1,
					 (int)reg);
			__raw_writel(reg, FRQCRB);
			memory_log_dump_int(PM_DUMP_ID_SET_FRQCRB_2,
					 (int)__raw_readl(FRQCRB));
			...
			/* apply setting */
			memory_log_dump_int(PM_DUMP_ID_SET_FRQCRA_1,
					 (int)reg);
			__raw_writel(reg, FRQCRA);
			memory_log_dump_int(PM_DUMP_ID_SET_FRQCRA_2,
					 (int)__raw_readl(FRQCRA));

			cpg_dbg_check_div_rates();
			/* set and wait for KICK bit changed */
			ret = cpg_set_kick(KICK_WAIT_INTERVAL_US);
			memory_log_dump_int(PM_DUMP_ID_SET_FRQCRA_3,
					 (int)__raw_readl(FRQCRA));
			if (ret) {
				if (zs_change) {
...
/* PM APIs ********************************************************************/
/******************************************************************************/
int __init pm_setup_clock(void)
{
	 ...
	 * Set AXI ZX clock to maximum frequency in early suspend mode
	 * until issue is fixed from modem side.
	 */
	if (shmobile_is_u2b())
		__shmobile_freq_modes_es2_x[FREQ_ES_MIN_MODE].zx_clk = DIV1_3;
...
#ifndef CONFIG_SH_THERMAL
	/*
	 * If thermal driver is not defined, put thermal sensor
	 * in idle state and stop the clock to save power in
	 * device suspend.
	 */
	do {
		unsigned int val;
		...
		/* enable ths clock */
		val = __raw_readl(SMSTPCR5);
		val &= ~(1 << SMSTPCR5_THS_STOP_BIT);
		__raw_writel(val, SMSTPCR5);

		/* put ths in idle state */
		__raw_writel(0x1303, THERMAL_THSCR0);
		__raw_writel(0x1303, THERMAL_THSCR1);

		/* 300us delay before clocks are taken out */
		udelay(300);
...
int pm_get_clock_mode(int mode, struct clk_rate *rate)
{
	...
	*rate = __shmobile_freq_modes[mode];
