...
static int ramdump_disabled;
static int disable_ramdump(const char *val,
		const struct kernel_param *kp)
{
	int ret = param_set_bool(val, kp);

	pr_err("%s: Ramdump %s\n", __func__, ramdump_disabled ? "disabled" : "enabled");
	if (ramdump_disabled)
		writeb((__raw_readb(STBCHR2) | APE_RESETLOG_RAMDUMP_DISABLED), STBCHR2);
	else
		writeb((__raw_readb(STBCHR2) & ~APE_RESETLOG_RAMDUMP_DISABLED), STBCHR2);
...
static struct kernel_param_ops disable_ramdump_param_ops = {
	.set = disable_ramdump,
	.get = param_get_uint,
};
module_param_cb(disable, &disable_ramdump_param_ops, &ramdump_disabled, 0644);

static struct ramdump *ramdump;
unsigned long *dummy_mask;

#ifndef CONFIG_ARM_PATCH_PHYS_VIRT
unsigned long __pv_phys_offset;
#endif
				      ...
				      enum kmsg_dump_reason reason)
{
	...
	*/
	...
	outer_flush_all();
	dsb();
	/* Do the dummy write to non-cached ram. This will write current
	 * processor id bit to dummy_mask shifted by reason. so for example
	 * that we can decode, if needed, which cores passed this point
	 * and what was the reason. This is only logging from undef to
	 * emerg so bits are sifted by emerg + 1 */
	*dummy_mask |=
		(BIT(reason) << ((KMSG_DUMP_EMERG + 1) * smp_processor_id()));
	dsb();
}
		...
		unsigned long event, void *ptr)
{
	...
	/* cache cleaning will happen in kmsg dump handler only if
	 * always_kmsg_dump is set but cache flush happens also just before
	 * doing reboot and going to ramdump mode so no need to do it here */
	emergency_restart();
...
{
	int ret = 0;
	unsigned long __iomem *start_addr;
...
#ifndef CONFIG_ARM_PATCH_PHYS_VIRT
	__pv_phys_offset = CONFIG_MEMORY_START;
#endif

	start_addr = ioremap_nocache(SDRAM_RAMDUMP_START_ADDR,
				     SDRAM_RAMDUMP_END_ADDR -
				     SDRAM_RAMDUMP_START_ADDR + 1);
	if (start_addr) {
		memset((void __force *) start_addr, 0xFF,
			SDRAM_RAMDUMP_END_ADDR - SDRAM_RAMDUMP_START_ADDR + 1);
		*start_addr = virt_to_phys(split_resources);
	}
	...
	dummy_mask = ramdump->reg_dump_base + ramdump->reg_dump_size -
			sizeof(*dummy_mask);
